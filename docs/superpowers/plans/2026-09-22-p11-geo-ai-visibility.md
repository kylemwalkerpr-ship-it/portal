...
- [ ] **Step 1: Write failing tests** for authoritative target selection, I-485/AU-485 separation via existing ownership truth, URL normalization/classification, known retired Marketplace path, wrong-owner citation, malformed response => `parse_failure`, mixed outcome coverage, and zero-success `shareOfVoice=null`.
- [ ] **Step 2: Run** `npx jest --runInBand tests/p11-geo-visibility-truth.test.ts` and verify RED because the P11 truth module/status behavior does not yet exist.
- [ ] **Step 3: Implement** the smallest pure contract and adjust `EngineAudit`/parser result so only a structurally parsed response is `success`; regex-recovered malformed output remains evidence with `status=parse_failure` and never enters citation math.
- [ ] **Step 4: Re-run** the focused test and the existing `seo-llm-visibility*` + P1 failure-accounting suites; require GREEN.
- [ ] **Step 5: Commit** `feat(seo): add P11 GEO visibility truth contract`.
...
- [ ] **Step 1: Write failing migration-contract tests** asserting additive nullable columns, closed status CHECK vocabulary, no historical backfill, broad legacy policy removal, `REVOKE ALL` from `anon`/`authenticated`, service-role server access, and no security-definer function.
- [ ] **Step 2: Run** the new DB-contract test plus migration-order/ledger-policy tests and verify RED on the missing migration.
- [ ] **Step 3: Add** the migration and update persistence to write all P11 fields while preserving legacy columns/`engines_json`.
- [ ] **Step 4: Re-run** DB-contract, migration-order, migration-ledger-policy, and LLM visibility suites; require GREEN.
- [ ] **Step 5: Commit** `feat(seo): persist secure P11 GEO evidence`.
...
- [ ] **Step 2: Run** the focused execution test and verify RED on current adaptive/fuzzy behavior.
- [ ] **Step 3: Wire** the strategic target selection and closed status model into `auditQuery`/`runVisibilityAudits`; retain explicit query text but require authoritative owner resolution for P11 measurement.
- [ ] **Step 4: Re-run** focused + existing audit/cron/action-stream route suites and require GREEN.
- [ ] **Step 5: Commit** `feat(seo): bind GEO audits to strategic owners`.
...
- [ ] **Step 2: Run** the focused test and verify RED against current fuzzy `content_jobs`/cluster fallback and generic action list.
- [ ] **Step 3: Replace runtime remediation matching** with exact owner-bound behavior; preserve old pure helpers only if existing callers/tests require them, but runtime P11 may not use fuzzy CREATE fallback.
- [ ] **Step 4: Re-run** remediation + LLM + Content Studio affected suites; require GREEN.
- [ ] **Step 5: Commit** `fix(seo): make GEO remediation evidence driven`.
...
- [ ] **Step 1: Write failing route/source tests** for all required counters, legacy rows remaining separate, zero-success null, and admin copy that names the successful-attempt denominator rather than a naked percentage.
- [ ] **Step 2: Run** reporting/status/admin tests and verify RED.
- [ ] **Step 3: Implement** aggregation/read-side reporting and the bounded admin evidence panel additions without changing unrelated UI.
- [ ] **Step 4: Re-run** focused reporting/status/admin suites; require GREEN.
- [ ] **Step 5: Commit** `feat(seo): expose P11 GEO evidence coverage`.
...
- [ ] **Step 1: Run** all P11/P1/ownership/provider/migration focused suites, `npx tsc --noEmit`, full `npm test -- --runInBand`, `npm run build`, and `git diff --check`; restore any generated artifacts that are not intentional.
- [ ] **Step 2: Record** exact local results and read-only production baseline in the execution ledger: 267 historical rows; 197 legacy measured under P1 rules; 70 `audit_failed`; 3 malformed; 6 no-sources; 0 measured estate citations; 55 distinct queries; latest observation 2026-09-15; 0 fan-out rows; insecure pre-P11 public policy/grants; two commissioned runtime providers only.
- [ ] **Step 3: Keep P11 non-PASS** unless post-deploy live P11 observations satisfy the canonical gate; explicitly retain public-answer-engine/provider limitations and P9/P10 open outcomes.
- [ ] **Step 4: Self-review the whole branch** because no independent executor/reviewer is available; fix Critical/Important findings with RED→GREEN tests, then re-run affected/full verification.
- [ ] **Step 5: Commit** `docs(seo): record P11 GEO visibility evidence`.
