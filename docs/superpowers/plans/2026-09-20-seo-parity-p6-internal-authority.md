...
- `lib/seoFactory/interlinkReconciliation.ts` reads at most 200 `status='planned'` rows that already carry a durable `source_url` **and** a valid exact `source_job_id` (query-filtered, so the unstaged backlog can never starve them);
...
- `verifyLiveUrl` ok=true is the only gate; finalization runs **job-bound** (`finalizeStagedInterlinksForLiveSource({ canonicalUrl, sourceJobId })` — only rows staged by that exact job), re-proves the exact live anchor + live target, and still updates `planned` rows only;
...
**Branch shape (truthful).** This branch is now **six commits above base** `e44fa01944423babe906c45f36c06bf4b6a8b4d4`: `e9bfe607` (foundation) → `c7f2b43d` (first supervisor-review repair) → `eb2b1133` (test-lifecycle hardening) → `f3e83e9f` (docs correction) → `403ba549` (second supervisor repair) → this ordinary third supervisor follow-up repair commit. The intended in-place amend of `e9bfe607` was denied by the runtime's Git broker (no destructive-git authority; no bypass attempted), so **nothing here is amended in place and the branch is not a single commit today**. Collapsing later needs no further local history rewrite: `git rebase -i --autosquash e44fa019` under destructive-git authority, or a normal **squash-merge** landing one commit on `main`.
...
3. Move rows from `planned` to proven `applied`: after a ship the immediate background path finalizes job-bound when the deployment already satisfies `verifyLiveUrl` (`verifyLiveInBackground` → `runBackgroundLiveVerification`), the **scheduled** pass re-verifies exact `(source_url, source_job_id)` subjects automatically once the deployment is observable (`reconcileStagedInterlinks` on the daily cron; no operator click required), and the admin `verify-published` path remains available; rows with no job identity (historical/backlog, or staged by a legacy caller) remain unresolved/manual and still need an authorized verification run.
