...
| P1 | Task 1 boundary: `support_notify(uuid,text,text,text,text,text)` and `support_log_action(uuid,text,text,text,text,jsonb)` are executable only by `service_role`, and `content_job_health_summary` / `inquiry_engagement` / `seo_backlink_dashboard` / `support_user_notes_v` are `security_invoker` views selectable only by `service_role` | IN_PROGRESS | Narrow claim only — does not assert base-table least-privilege (tracked in the next row). 2026-09-15 local acceptance: `tests/support-security-boundary.test.ts` 3/3 pass (exact `to_regprocedure`/`to_regclass` guards; revoke from `public, anon, authenticated`; `grant execute`/`grant select` only to `service_role`; `security_invoker = true`); `tests/migration-order.test.ts` 8/8 pass; `npx tsc --noEmit` exit 0. Live read-only proof still shows both support SECURITY DEFINER RPCs executable by `anon`/`authenticated` (Postgres 17.6). Migration is prepared but production DDL has not been applied, so this cannot be PASS until post-merge privilege verification proves `anon/authenticated=false`, `service_role=true`. |
