# Content Studio — executor-policy audit (v8 history with v9 revision note)

Date: 23 September 2026. Input: historical CS-2026.09.23.7 at `2af720e8290c67806f335da746f91af35dd2c5f6`. Corrected design: **CS-2026.09.23.8**. The v8 findings below remain historical and unchanged.

**v9 revision note (2026-09-23):** A narrowly scoped executor-policy update records local Codex CLI 0.156.1 via RDC, model ID `gpt-6-luna`, five successful strict read-only one-shot effort probes (low through max), and one catalog-refresh timeout with explicit invocation success. Policy schema v3 separates observed capability from per-task lowest-sufficient effort selection. A0.1 PR #283 merged at `22c685e38d46367eec18332f264b7f1480d8305e`; workflow 35822388398 succeeded; Worker `b5e39685-baa2-4789-acfa-10e7f990d2ad`, build `LU03P9-X1WyePB7inlqsO`; production health passed and exact-main smoke passed 4/4. A0.1 authorized no broader implementation. P9–P11 remain IN_PROGRESS, P12–P13 PENDING; Worker CPU is UNVERIFIED. No A0.2/A0.3 admission, refactor, DB change, CREATE, publication, or phase promotion. These are documentation and release-evidence updates only; no phase snapshot or Worker budget evidence was changed. Bounded validation: JSON parse and AJV 6 structural/conditional validation passed after in-memory Draft-7 compatibility normalization; 11 negative fixtures rejected. Exact-byte SHA-256 manifest, 28-section sequence, 34 unique interface declarations, all 50 ordered SEO keys, and Markdown fence checks passed. A full Draft 2020-12 validator was not available; no package was installed.

**Historical v8 verdict:** A0.1 documentation-integrity correction ready; runtime performance UNVERIFIED and implementation entry still gated. No code, database migration, provider/account probe, production probe, phase promotion or deployment was performed by that amendment. The v9 note above updates policy and release evidence only; it does not advance a program or runtime gate.

## Evidence and decisions

The attached Sol conversation (`Pasted markdown(1).md`) was read in full. It is an investigation source, not fresh runtime proof. Current Portal main was read at `3d916b09671ab726de2fc1aa3b87f79e7712c6ac`; parity blob `0d2e1f5ed78adaf48334f87a21caa9a602e398cd` preserves open outcomes. The user's P0–P11 code-PASS correction is retained separately. A read-only Luna/high review assessed the worker boundary; the primary author checked and integrated its findings. **Historical v8 executor observation:** the Codex catalog advertised `gpt-5.6-luna` efforts `low`, `medium`, `high`, `xhigh`, `max`; only `gpt-5.6-luna/high` positively succeeded for that v8 candidate. Future packet admission must re-resolve the exact Luna model and catalog, select and record a supported effort/rationale, verify selected-effort access, use `RDC_CODEX_CLI`, and record native-nested versus separate-CLI orchestration; no automatic model/provider-family fallback.
...
| F25: old executor defaults conflict with user direction | Sol supervises; local Codex CLI model gpt-6-luna runs through RDC at the per-task lowest sufficient verified effort; distinct review; explicit orchestration; no silent substitution or fallback | T30 packet/capability checks; existing external jobs untouched |
...
## Historical v8 verification scope
