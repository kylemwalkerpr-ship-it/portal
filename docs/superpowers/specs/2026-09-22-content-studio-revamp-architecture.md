...
**Version CS-2026.09.23.9** · Normative master design · **Astra design audit: APPROVED; implementation remains BLOCKED**
Owner: Kyle · Architecture and release authority: GPT Sol · Executor: local Codex CLI 0.156.1 via RDC, model ID `gpt-6-luna`, with dynamically selected task effort
Supersedes CS-2026.09.23.8 as delivered in main commit `22c685e38d46367eec18332f264b7f1480d8305e`. That v8 revision superseded CS-2026.09.23.7 at source commit `2af720e8290c67806f335da746f91af35dd2c5f6` (the v7 integrity/executor-pin baseline), which superseded CS-2026.09.22.6 at architecture branch head `c23ed7e4c687eeca59933e46e1e37e796afea681`. Earlier revisions are historical. `docs/superpowers/content-studio-revamp.md` remains a non-normative pointer. This file is the sole normative architecture; the companion `docs/superpowers/plans/2026-09-22-content-studio-revamp-blueprint.md` translates it into implementation packets without independent policy authority. The audit record is `docs/superpowers/specs/2026-09-22-content-studio-revamp-final-audit.md`.

Approval means the design corrections are accepted for the gated implementation sequence. It is not a claim that live capabilities, reviewer availability, phase outcomes, runtime safety or account budgets have passed. Sol retains scope, approval, review, integration and merge authority. The current executor pin is local Codex CLI 0.156.1 through RDC, model ID `gpt-6-luna`; each admitted packet re-resolves the current catalog and explicitly probes/selects the lowest sufficient supported effort, with access verified. Sol delegates bounded packets and independently reviews integration; no automatic provider-family fallback is authorized for this refactor.
...
### 1.2 Authority order
Current human authorization → repository `AGENTS.md` and approved design → Supervisor Arc current GPT-6 Luna execution policy and adaptive effort selection → SEO Brief and the current repository parity matrix → this document. Conflicts block the affected action. This document does not amend phase thresholds and does not authorize broad CREATE, outreach, destructive action, merges or deployments by itself.
...
**v9 executor-policy correction (2026-09-23):** Current pin: local Codex CLI 0.156.1 via Remote Desktop Commander (RDC), model ID `gpt-6-luna`. Five strict read-only one-shot invocations returned OK/exit 0 for `low`, `medium`, `high`, `xhigh`, and `max`; one catalog refresh timed out, while explicit model invocation completed successfully. These are dated capability observations, not future admission guarantees. At every task admission, re-check the current model catalog and explicitly probe the selected effort; choose the lowest effort sufficient for task risk and complexity from currently supported levels, up to the current maximum `max`, without a global `high` or `max` pin. Never silently substitute a model, family, or effort; if GPT-6 Luna or the selected effort is unavailable, stop with `BLOCKED_CAPABILITY`. Use native Codex subagents only after verifying native availability and exact model/effort support; otherwise use separate RDC-launched CLI sessions and record `nativeChild=false`. Sol owns scope, approval, independent review, integration, and merge. Provide authorized GitHub/Supabase/Cloudflare integrations to the executor and verify calls work inside it; capability is not operational authorization. Sol owns safe Git fetch, fast-forward, and parity checks and confirms local/remote parity before task work; preserve dirty, diverged, and other-owned worktrees and never force reset. Code work stays in a scoped worktree/branch and reviewed PR with required checks and official Actions deployment. A main push deploys production; a documentation PR merge also triggers the Worker deployment; disclose that release consequence and follow the authorized merge/deploy path. A0.1 authorized no broader implementation. Gates remain P9–P11 IN_PROGRESS, P12–P13 PENDING; Worker CPU remains UNVERIFIED. There is no A0.2/A0.3 admission, refactor, database change, CREATE, publication, phase promotion, or gate bypass. This documentation-only correction advances no gate and implies no runtime implementation.
...
interface ExecutorPolicy {
  schemaVersion: "studio.executor-policy/3";
  ...
  effortCapability: { advertised: ReasoningEffort[]; verifiedInEnvironment: ReasoningEffort[];
    probeMode: "STRICT_READ_ONLY_ONE_SHOT"; probeResults: EffortProbe[];
    catalogRefreshObservation: "SUCCEEDED" | "TIMEOUT_WITH_EXPLICIT_INVOCATION_SUCCESS";
    explicitInvocationCompleted: boolean };
  effortSelection: { mode: "LOWEST_SUFFICIENT_PER_TASK_ADMISSION";
    taskAdmission: { selectedEffort: ReasoningEffort; selectionRationale: string;
      selectedEffortAccess: "VERIFIED" } | null };
...
interface EffortProbe { effort: ReasoningEffort; result: "OK"; exitCode: 0; }
```
...
**D4 executor route:** deterministic capability/cost registry in v6. Route exact parsing/validation to rules, batch semantics to the commissioned Modal resource class, difficult production research to a commissioned model or qualified human; development/refactor work to local Codex CLI model gpt-6-luna through RDC at the task-selected lowest sufficient supported effort, architecture/arbitration to Sol, and consequential approval to a qualified human. Never pay to run a lower-tier model for a task known to require human expertise. Capability availability/cost evidence selects among permitted alternatives; there is no silent provider substitution.
...
- Every Sol packet states: task id/revision, absolute worktree, verified base, objective, scope/exclusions, dependencies, interfaces consumed, acceptance, source references, capabilities, checkpoint conditions and recovery record. The pinned local Codex CLI via RDC performs investigation, refactor and verification at the task-selected lowest sufficient verified effort in disjoint bounded worktrees; a distinct reviewer inspects critical changes and Sol independently reviews design, a representative first change and the final candidate. Record whether orchestration used native nested tools or a separate RDC/local CLI session; never describe the latter as a native child. Re-resolve model and effort at admission, use no automatic provider-family fallback, and reconcile existing agent/task state before retrying a timeout.
...
Additional delivery cases T25–T30 are defined in blueprint §15.3 and are binding: auth/anon/cold-key CPU cohorts; lost-submit/reconnect idempotency; sequenced Tape; optional Queue recovery/retention; code/gate separation; GPT-6 Luna packet admission with per-task lowest-sufficient verified effort with verified selected effort, explicit transport, and distinct orchestration mode.
...
Sol commissions each source and confirms account entitlement; the pinned local Codex CLI via RDC implements and verifies adapters at the lowest sufficient, admission-time probed effort under Sol's scoped packets. This specification purchases nothing and assumes no working credentials. Providers are wired per §16.2; the Modal analyzer parses Markdown/HTML **once per artifact hash** and computes all deterministic metrics from the shared AST/tokenization (model calls only for ambiguous semantics; Jev is never used for simple counts).
...
### 27.4.3 Historical CS-2026.09.23.8 A0.1 integrity/executor-pin correction
...
### 27.4.4 CS-2026.09.23.9 executor-policy schema update
This concise v9 amendment updates the executor pin to local Codex CLI 0.156.1 via RDC and `gpt-6-luna`, records five dated strict read-only one-shot effort probes plus the catalog-refresh timeout, and separates available effort evidence from each task's admission-time selection. It preserves release facts, phase gates, Worker CPU status, SEO constraints, and all v8 audit findings as historical evidence. A0.1 authorized no broader implementation; this amendment authorizes no implementation, database work, provider/account probes, CREATE, publication, phase promotion, deployment, or gate bypass.
