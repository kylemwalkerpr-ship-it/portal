...
**Version CS-2026.09.24.1** · Normative master design · **A0 and scoped implementation entry OPEN; action and production gates remain conditional**
Owner: Kyle · Architecture and release authority: GPT Sol · Current refactor executor: GPT-6 Luna through RDC/local Codex CLI, with task-selected freshly probed reasoning effort
Supersedes CS-2026.09.23.8 released at merge `22c685e38d46367eec18332f264b7f1480d8305e`; CS-2026.09.23.9 remains an unmerged, separately reconciled candidate. The v8 release superseded CS-2026.09.23.7 at source commit `2af720e8290c67806f335da746f91af35dd2c5f6` (the v7 integrity/executor-pin baseline), which superseded CS-2026.09.22.6 at architecture branch head `c23ed7e4c687eeca59933e46e1e37e796afea681`. Earlier revisions are historical. `docs/superpowers/content-studio-revamp.md` remains a non-normative pointer. This file is the sole normative architecture; the companion `docs/superpowers/plans/2026-09-22-content-studio-revamp-blueprint.md` translates it into implementation packets without independent policy authority. The audit record is `docs/superpowers/specs/2026-09-22-content-studio-revamp-final-audit.md`.

Approval means the design corrections are accepted for the gated implementation sequence. It is not a claim that live capabilities, reviewer availability, phase outcomes, runtime safety or account budgets have passed. Sol retains implementation supervision and merge authority; the Luna family executes each admitted packet at its task-selected supported effort after exact model and selected-effort access are re-resolved and verified. Sol delegates bounded packets and independently reviews integration; no automatic provider-family fallback is authorized for this refactor.
...
- **Historical v8 entry rule, superseded by §24.1 (2026-09-24):** Architecture authoring/review was permitted, but operational A0 and implementation awaited every SEO program gate. Current user direction admits scoped A0 and dependency-ordered construction while open P9–P13 gates retain their action-specific force. P13 completion is never a universal CREATE permit: each cluster/action still requires eligibility.
...
**Historical v7 current-state amendment (2026-09-23; source CS-2026.09.23.7):** Kyle confirms P0–P11 code complete to PASS; distinguish code acceptance from deployment and external/outcome gates. Current main was re-pinned to `3d916b09671ab726de2fc1aa3b87f79e7712c6ac`; the matrix blob is unchanged and retains P9/P10/P11 IN_PROGRESS. §8.1.1 defines provenance and remaining evidence. Do not redo accepted code merely because a program gate is open. This historical amendment updates documentation and executor standing orders; it starts no refactor job or production mutation.

**Historical v8 A0.1 integrity/executor-policy correction (2026-09-23; its engineering-entry hold is superseded by §24.1):** The dated Codex catalog advertised `gpt-5.6-luna` efforts `low`, `medium`, `high`, `xhigh` and `max`; only the selected `high` effort was positively exercised for this packet. Every future packet must re-resolve the exact current Luna model ID and advertised effort catalog at admission, select a supported effort for the task, record the rationale, and verify access to that selected effort before execution. RDC/local Codex CLI is the standard refactor route; native nested-agent capability and a separate CLI session are distinct modes, and a CLI session is never represented as a native child. No automatic provider-family fallback is permitted. The dated delivery state records the packet's `gpt-5.6-luna`, selected `high`, verified `high`, `RDC_CODEX_CLI`, separate-session orchestration, and `automaticFallback=false`. While program gates remain open, this A0.1 packet permits documentation-integrity repairs and read-only readiness reconciliation under explicit scoped authorization only. It does not authorize operational A0.2–A0.7, application implementation, database/provider/account probes, deployment, CREATE/publication, phase promotion or gate bypass. Operational A0 and implementation remain entry-gated unless separately explicitly authorized. This is a documentation-only correction and does not advance any gate or imply runtime implementation.
...
interface ExecutorPolicy {
  schemaVersion: "studio.executor-policy/2";
  supervisor: "GPT_SOL"; executor: "CODEX_LUNA"; executorFamily: "LUNA";
  modelResolution: { mode: "RE_RESOLVE_AT_ADMISSION"; resolvedModelId: string;
    resolvedAt: string; evidence: string };
  effortCatalog: { advertised: ReasoningEffort[]; verifiedInEnvironment: ReasoningEffort[] };
  selectedEffort: ReasoningEffort; selectionRationale: string; selectedEffortAccess: "VERIFIED";
  transport: "RDC_CODEX_CLI";
  orchestration: { mode: "NATIVE_NESTED_AGENT" | "SEPARATE_CLI_SESSION";
    nestedCapability: "AVAILABLE_AND_USED" | "AVAILABLE_NOT_USED" | "UNAVAILABLE" | "NOT_PROBED";
    nativeChild: boolean };
  automaticFallback: false; modelId: string; maxConcurrentAgents: number;
  writerConflictDomain: "ONE_WRITER";
  reviewerIdentity: "DISTINCT_EXECUTOR";
...
Implementation executor policy is development-time authority, not a production generation provider. Luna CLI execution is never a persistent backend dependency. At admission, each packet re-resolves the exact model ID and advertised effort catalog, selects a supported effort for the task, records why that effort is adequate, and verifies access to it. Each executor receives the resolved model/effort, transport (`RDC_CODEX_CLI`), orchestration mode, pinned base/worktree, bounded paths, dependencies, acceptance and prohibited side effects. Native nested tools and separate CLI sessions are recorded distinctly; a separate CLI session is not a native child. Missing Luna/model/selected-effort capability yields `BLOCKED_CAPABILITY`, with no automatic provider-family fallback. Start with ≤3 concurrent workers (and never above available capacity), one writer per conflict domain, a distinct independent reviewer, and Sol integration/merge. Existing external sessions remain untouched.
...
**D4 executor route:** deterministic capability/cost registry in v6. Route exact parsing/validation to rules, batch semantics to the commissioned Modal resource class, difficult production research to a commissioned model or qualified human; development/refactor work to the Luna family at the task-selected supported effort, architecture/arbitration to Sol, and consequential approval to a qualified human. Never pay to run a lower-tier model for a task known to require human expertise. Capability availability/cost evidence selects among permitted alternatives; there is no silent provider substitution.
...
### 24.1 Gate rule
**User-authorized staged entry (2026-09-24): Sol may now open bounded A0.2–A0.7 investigation and dependency-ordered A1–A7 construction without first closing P9–P13.** A0.1 was released at `main@22c685e38d46367eec18332f264b7f1480d8305e`; reverify its artifact integrity against current main before consuming it. Every packet pins fresh `origin/main`, its authorized paths, read/write capability, cost ceiling, evidence and independent reviewer. No dependent A1–A7 packet begins until its required A0 sub-package exits with recorded evidence. A0 exit evidence is stored (T1/T2) and referenced by packet ID; summaries are insufficient. Stateful A0 capability probes (including Modal Volume create/mount/read-write checks), provider invocation, provisioning and spend require a separate bounded authorization, evidence and cleanup/reconciliation; read-only entry alone cannot mark A0.7 complete. A failed or uncertain probe blocks only its dependent work. The existing 1102 investigation and current production responsibilities continue independently; inventory/reuse its receipts instead of duplicating fixes.

**Build admission is not production or content permission.** Read-only A0 probes and reviewed code/tests on isolated branches are admitted under the scoped packet process; any production database mutation, credential/provisioning action, provider invocation or spend, merge, deployment, external write, CREATE or publication requires its own applicable authority and release evidence. P9's verified third-party backlink, P10's real paid order, and P11's real GEO observation remain open outcome gates; never synthesize them or reset passing phase code. P12 remains the estate validation/activation gate: affected paths must demonstrate functional, security, cost and total Worker CPU acceptance with current evidence before activation or cutover. P13 remains the separately eligible cluster/action gate for net-new CREATE and publication; a P13 program PASS alone cannot unlock a page. Research, estate mapping, existing-owner audits, briefs, drafts and nonpublishing comparison can proceed under policy without minting a permit. Existing-content edits and technical repairs still require their applicable owner, YMYL and release permits. Sol's first implementation packet is A0.2 runtime/Cloudflare baseline after fresh main/PR/worktree reconciliation; A0.3 database/capacity baseline may run in a disjoint domain with a distinct reviewer. No import of heavy crawlers, parsers, generators, Clerk or Supabase admin clients into the anonymous HTTP hot path; heavy work is admitted as durable commands to Modal, subject to A0 capability proof. Worker CPU <10ms total invocation, p95 ≤2.5ms and p99 ≤5ms stay unverified until Cloudflare cohort telemetry confirms them.

**CMS acceptance, not volume:** every proposed intervention starts from source-provenance, topical/reader-job identity, jurisdiction and estate owner state; may end in NO_ACTION, technical fix, authority work, consolidation, refresh or a reviewed new page. The editorial contract binds grounded claims, original information gain, YMYL human approval, accessible final render and exact canonical/slug reservation before a release permit. Jev ranks eligible opportunities but cannot grant permissions. The system measures live discoverability, useful engagement, qualified Marketplace outcomes and mature rewards, with separate known/unknown denominators. No article quota, text volume or model confidence is an acceptance substitute. These are acceptance requirements for future packets, not claims that the CMS is already implemented.
...
- Every Sol packet states: task id/revision, absolute worktree, verified base, objective, scope/exclusions, dependencies, interfaces consumed, acceptance, source references, capabilities, checkpoint conditions and recovery record. Luna performs investigation, refactor and verification at the task-selected verified effort in disjoint bounded worktrees; a distinct reviewer inspects critical changes and Sol independently reviews design, a representative first change and the final candidate. Record whether orchestration used native nested tools or a separate RDC/local CLI session; never describe the latter as a native child. Re-resolve model and effort at admission, use no automatic provider-family fallback, and reconcile existing agent/task state before retrying a timeout.
...
Additional delivery cases T25–T30 are defined in blueprint §15.3 and are binding: auth/anon/cold-key CPU cohorts; lost-submit/reconnect idempotency; sequenced Tape; optional Queue recovery/retention; code/gate separation; adaptive Luna packet admission with verified selected effort, explicit transport, and distinct orchestration mode.
...
Sol commissions each source and confirms account entitlement; Luna implements and verifies adapters at the task-selected verified effort under Sol's scoped packets. This specification purchases nothing and assumes no working credentials. Providers are wired per §16.2; the Modal analyzer parses Markdown/HTML **once per artifact hash** and computes all deterministic metrics from the shared AST/tokenization (model calls only for ambiguous semantics; Jev is never used for simple counts).
...
### 27.4.2 Historical CS-2026.09.23.7 Worker and executor amendment
...
### 27.4.3 CS-2026.09.23.8 A0.1 integrity/executor-pin correction
This v8 revision supersedes the historical v7 source at `2af720e8290c67806f335da746f91af35dd2c5f6`. It corrects the dated executor observation and exact-byte manifest integrity, preserves the architecture and its contracts, and adds the narrowly scoped documentation-only exception while program gates remain open. It does not authorize operational A0.2–A0.7, application implementation, database/provider/account probes, deployment, CREATE/publication, phase promotion or gate bypass, and it advances no program or runtime status.

### 27.4.4 CS-2026.09.24.1 staged implementation entry
The open v9 GPT-6 Luna executor amendment is carried forward as current packet policy without representing its PR as merged; the v8 gpt-5.6-luna A0.1 observation remains historical in the dated delivery state. The user's 2026-09-24 direction supersedes only the v6/v8 global program-entry hold. §24.1 now admits scoped A0 and dependency-ordered engineering while P9–P11 outcomes remain open, P12 gates validation/activation and P13 gates cluster-specific net-new CREATE. Existing runtime CPU, qualified reviewer, provenance, security, release and action-permit thresholds remain unchanged. A0.1 v8 is released; the v9 docs branch and dependent P11 evidence branch are separate open candidates that must be reconciled by their owners before any later docs merge. The change is an entry-policy amendment, not phase promotion or a claim of Worker performance.
