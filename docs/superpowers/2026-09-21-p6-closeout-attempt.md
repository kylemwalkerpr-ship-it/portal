...
## Final production reconciliation addendum

The earlier credential-blocked snapshot above is superseded for current production state.

- The repository Batch A apply path was subsequently run with genuine legacy service-role authority, preserving the exact confirmation token, <=200-row hard limit, fresh 404/410 GET re-confirmation, and CAS fences.
- Final production state: **1,494 rejected · 405 planned · 0 applied** out of 1,899 rows.
- Final dry run after cleanup: **0 dead candidates**, 73 live Marketplace rows, 332 legacy Portal-wall rows, 0 unknown.
- All 1,494 rejections are audited as `stale_target_http_404` by `p6-batch-a-stale-rejection`; no source/job/proof/applied fields were fabricated.
- Post-cleanup canonical disposition still yields **approvedUseful 0/73 verified = 0.0%**, so P6 remains **IN_PROGRESS / BLOCKED**, not PASS.
- Durable source-authority resolution found only 1/73 content-job mapping and 0 anchor-ledger job bindings. At least 59/73 genuine live-source proofs are required for the >=80% gate; those identities cannot be guessed.
