...
2. Finalization is wired to the admin `verify-published` path; ship-time verification remains fire-and-forget (`verifyLiveInBackground`) and therefore does not finalize staged rows on its own — staged rows wait for a verify-published call (or a later scheduled verifier).
