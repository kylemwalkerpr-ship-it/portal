...
Residual scope note (not a blocker in this repair, disclosed for the next pass): the complete-flow noindex fixer (`fixNoIndexPagesChunked()` in `siteHealthComplete.ts`) is a separate mutation path that this repair did not extend registry protection to. No cohort URL carries a noindex directive today (audit observation: robots allowed, HTTP 200), so it cannot currently alter a protected page, but it should be covered when the complete-flow repair surface is next touched.
