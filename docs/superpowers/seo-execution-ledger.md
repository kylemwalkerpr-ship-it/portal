...
2. ~~Finalization is wired only to the admin `verify-published` path; ship-time verification remains fire-and-forget and does not finalize staged rows on its own.~~ **Repaired in the supervisor-review amend:** the normal background path now finalizes (`verifyLiveInBackground` → `runBackgroundLiveVerification` → `finalizeStagedInterlinksForLiveSource` for the exact verified canonical), and both ship success paths stage BEFORE launching verification. Residual that remains: the background finalizer runs in the same fire-and-forget task as the ship's live verification, so a process crash between staging and verification still leaves rows `planned` until the next verification for that source (a later scheduled/admin verifier re-runs it; nothing is claimed applied meanwhile).
