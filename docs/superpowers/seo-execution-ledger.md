...
3. **M2 — the finalizer could overwrite a reship rebind.** Every job-bound verdict/applied write is now a compare-and-set on the EXACT selected subject (`id` + `status='planned'` + exact `source_job_id` + the row's exact `source_url`) in addition to the planned fence, including the source-fetch-failure verdict path. A concurrent rebind yields zero affected rows and is counted `skipped` — it can never stamp the old job back or apply over the new revision. Source-url-only legacy/admin finalization without a job id keeps the documented legacy behavior.
