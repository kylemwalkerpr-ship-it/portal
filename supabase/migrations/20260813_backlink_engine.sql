...
--      seo_anchor_ledger).
--   2. EXTERNAL backlinks (managed here) tracked in seo_backlink_targets +
--      seo_backlink_outreach + a deterministic dashboard view.
--
-- Also widen seo_interlinks.status to include the closed set the compliance
-- gate engine writes (manual / rejected / paused / awaiting_gate).
-- ============================================================================

DO $$
BEGIN
  IF EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'seo_interlinks_status_check'
  ) THEN
...
BEGIN
  IF EXISTS (
    ...
    WHERE table_schema='public' AND table_name='seo_interlinks'
  ) THEN
    ALTER TABLE public.seo_interlinks
      ...
      NOT VALID;
    UPDATE public.seo_interlinks
      SET status='planned'
      WHERE status IS NULL OR status NOT IN ('planned','applied','rejected','manual','paused','awaiting_gate');
...
CREATE TABLE IF NOT EXISTS public.seo_backlink_outreach (
  ...
  status TEXT NOT NULL DEFAULT 'drafted' CHECK (status IN
    ('drafted', 'queued', 'sent', 'bounced', 'responded',
...
SELECT
  ...
  (
    SELECT count(*)::int FROM public.seo_backlink_outreach o
    WHERE o.target_id = t.id AND o.status = 'won'
  ) AS wins
FROM public.seo_backlink_targets t;

-- Seed default curated target list. Reduced to single-line values to keep
-- CHECK constraint compilation fast.
...
VALUES
  ('uscis.gov', 'https://www.uscis.gov/', 'USCIS (US Citizenship and Immigration Services)', 'gov', 'editorial', 95.0,
   ARRAY['US'], ARRAY['schools','work','visa','settlement','citizenship','family'],
   ARRAY['f-1','h-1b','i-485','n-400','green-card'],
   'Direct policy source. Earned citations in casework and FAQ content when we cite USCIS forms, processing-time data, and statutory anchors.',
   'uscis.gov'),

  ('travel.state.gov', 'https://travel.state.gov/', 'US Department of State - travel and visas', 'gov', 'editorial', 94.0,
   ARRAY['US'], ARRAY['work','visa','settlement','family'],
   ARRAY['h-1b','l-1','k-1','cr-1','ds-160'],
   'Statutory anchor for consular processing. Earned backlinks by maintaining accurate DS-160 / NVC walkthroughs.',
   'travel.state.gov'),
  ...
  ('homeoffice.gov.uk', 'https://www.gov.uk/government/organisations/home-office', 'UK Home Office', 'gov', 'editorial', 92.0,
   ...
   'Statutory anchor for UK Immigration Rules and Appendix FM. Citations in skilled-worker and family visa guides.',
   'homeoffice.gov.uk'),
  ...
  ('canada.ca', 'https://www.canada.ca/en/services/immigration-citizenship.html', 'IRCC - Government of Canada', 'gov', 'editorial', 92.0,
   ...
   'Statutory anchor for IRPA and IRPR. Citations in Express Entry and study permit guides.',
   'canada.ca'),
  ...
  ('immi.homeaffairs.gov.au', 'https://immi.homeaffairs.gov.au/', 'Department of Home Affairs (AU)', 'gov', 'editorial', 91.0,
   ...
   'Statutory anchor for Migration Regulations. Citations in skilled and partner visa guides.',
  ...
  ('blog.google', 'https://blog.google/products/search/', 'Google Search Central Blog', 'media', 'editorial', 96.0,
   ...
   'Earned citations when we reference Google AI search and SGE guidance in our SEO strategy and E-E-A-T posts.',
   'blog.google'),
  ...
  ('developers.google.com', 'https://developers.google.com/search', 'Google Search Central Docs', 'media', 'editorial', 95.0,
   ...
   'Earned citations when our technical-SEO content covers structured data, indexing API, and page experience signals.',
  ...
  ('searchengineland.com', 'https://searchengineland.com/', 'Search Engine Land', 'media', 'guest_post', 88.0,
   ...
   'High-authority SEO publication. Guest posts on AI search and programmatic SEO for legal and immigration verticals.',
  ...
  ('ahrefs.com', 'https://ahrefs.com/blog', 'Ahrefs Blog', 'media', 'guest_post', 90.0,
   ...
   'Industry pillar. Guest posts on keyword research and content gaps for legal and immigration industries.',
   'ahrefs.com'),
  ...
  ('rcic.ca', 'https://www.rcic.ca/', 'Regulated Canadian Immigration Consultants Association', 'industry_blog', 'editorial', 75.0,
   ...
   'Industry association. Earned citations when we reference CRS changes and RCIC certification in our CA guides.',
   'rcic.ca'),
  ...
  ('mara.gov.au', 'https://www.mara.gov.au/', 'MARA - Migration Agents Registration Authority', 'industry_blog', 'editorial', 76.0,
   ...
   'Industry authority. Earned citations when we reference skills lists and MARA-registered agents.',
   'mara.gov.au'),
