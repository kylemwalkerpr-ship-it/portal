...
/**
 * Public suggest-brief transaction boundary.
 * Reservation happens before any research/brief model call. The existing core
 * assembler remains the one brief implementation; this route persists the exact
 * evidence snapshot + returned sealed brief as an immutable WritingContractV2.
 */
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const resolvedRegion = resolveBriefRegion(
      typeof body.region === 'string' ? body.region : null,
      `${topic} ${primaryKeyword}`,
    )
    const region = resolvedRegion.region
    const audience = String(body.audience || '').trim()
    const audienceStage = String(
      body.audienceStage || body.opportunity?.audienceStage || body.opportunity?.stage || '',
    ).trim() || undefined
    const userId = String(
    ...
    const priorOpportunity = body.opportunity && typeof body.opportunity === 'object'
      ? body.opportunity
      : {}
    body.opportunity = {
      ...priorOpportunity,
      signals: [
        ...(Array.isArray(priorOpportunity.signals) ? priorOpportunity.signals.map(String) : []),
        tinyfishBlock,
      ],
    }
    body.region = region
    ...
    const interlinks = Array.isArray(payload.interlinkTargets)
      ? payload.interlinkTargets
          .map((link: any) => ({
            ...
            placement: link?.placement ? String(link.placement) : undefined,
          }))
    ...
    const contract = await finalizeSuggestBriefContract({
      ...
      contentType,
      region,
      audience: String(payload.recommendedAudience || audience || topic),
      audienceStage,
      sealedBrief,
      ...
      minWords: Number(payload.minWords || 0),
      targetWords: Number(payload.targetWords || 0),
      maxWords: Number(payload.maxWords || 0),
      sources: Array.isArray(payload.sources) ? payload.sources.map(String) : [],
      interlinks,
      title: String(payload.suggestedH1 || body.title || primaryKeyword),
      targetSlug: String(payload.targetSlug || '').trim(),
      ...
      gsc: {
        impressions: Number(body.gscImpressions) || 0,
        clicks: Number(body.gscClicks) || 0,
        position: Number(body.gscPosition) || 0,
      },
    ...
    if (error instanceof OpportunityAlreadyReservedError) {
      return NextResponse.json({
        ok: false,
        error: error.message,
        code: 'opportunity_reserved',
        jobId: error.jobId,
        contractId: error.contract?.contractId || null,
        contractHash: error.contract?.contractHash || null,
    ...
    const status = /brief_invalid|writing contract validation/i.test(message) ? 422 : 500
