...
export async function PATCH(_req: Request, context: { params: Promise<{ id: string }> }) {
  ...
  const { data: updated, error } = await auth.db.from('gigs').update({ status: 'archived', updated_at: new Date().toISOString() }).eq('id', id).select('*').single()
  if (error || !updated) return fail(error?.message || 'Could not archive gig.', 500)
