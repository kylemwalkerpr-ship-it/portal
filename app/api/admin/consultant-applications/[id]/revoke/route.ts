/**
 * POST /api/admin/consultant-applications/[id]/revoke
 * Revoke an already-approved consultant. Flips the application to declined,
 * suspends the profile, pauses gigs, writes an audit event, and emails
 * the consultant that access has been revoked.
 */
import { getClerkUserId } from '@/lib/auth'
import { createSupabaseAdminClient } from '@/lib/supabase'
import { sendEmail, consultantDeclineEmail } from '@/lib/email'
...
export async function POST(req: Request, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    await sendEmail({
      ...
      subject: 'Your YouSafe consultant access has been revoked',
      html: email.html,
    })
  } catch (err) {
    console.error('[consultant-applications/revoke] email failed', err)
  }
