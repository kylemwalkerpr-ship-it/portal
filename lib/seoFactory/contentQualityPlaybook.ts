...
export const CONTENT_QUALITY_PLAYBOOK: readonly GateDefinition[] = [
  ...
  def({
    code: 'keep_chrome_leak', title: 'KEEP chrome glued onto YAML fence or script tag',
    severity: 'format_blocker', owner: 'deterministic', repairClass: 'deterministic',
    appliesTo: INDEXABLE_FORM,
    requirement: 'No KEEP--- / KEEP<script / KEEP&lt;script tokens in the persisted document.',
    promptInstruction: 'Never prefix --- or <script> with KEEP. Leave fences and schema tags unchanged.',
    evidence: 'contentQualityGate.evaluateContentQuality KEEP chrome scan',
    shipEffect: 'block', evaluator: 'contentQualityGate.evaluateContentQuality',
    testFixture: 'tests/keep-fence-normalize.test.ts',
  }),
  def({
    code: 'escaped_script_leak', title: 'HTML-escaped script / JSON-LD visible in body',
    severity: 'format_blocker', owner: 'deterministic', repairClass: 'deterministic',
    appliesTo: INDEXABLE_FORM,
    requirement: 'Schema must be a real <script type="application/ld+json"> block, never &lt;script&gt; text.',
    promptInstruction: 'Unescape or remove entity-encoded script blocks so they are not visible prose.',
    evidence: 'contentQualityGate.evaluateContentQuality escaped script scan',
    shipEffect: 'block', evaluator: 'contentQualityGate.evaluateContentQuality',
    testFixture: 'tests/keep-fence-normalize.test.ts',
  }),
  def({
  ...
  def({
    ...
    promptInstruction: 'Add ## In 60 seconds with 3–5 direct takeaway bullets, one `- ` item per line. Emit this heading exactly once — never a second In 60 seconds section.',
  ...
  def({
    ...
    severity: 'warning', owner: 'writer', repairClass: 'targeted_ai', appliesTo: INDEXABLE_FORM,
    requirement: 'Developed 4–6 sentence paragraphs are allowed. Flag only 7+ sentence walls over ~720 characters.',
    promptInstruction: 'Break a 7+ sentence wall into shorter units, a list, step, table, or callout. A developed 4–6 sentence paragraph is allowed.',
  ...
  def({
    ...
    promptInstruction: 'Add ## In 60 seconds with 3–5 direct bullets. One section only.',
  ...
  def({
    code: 'stuffed_primary_opener', title: 'Section opens by restating the primary keyword',
    severity: 'warning', owner: 'writer', repairClass: 'targeted_ai', appliesTo: INDEXABLE_FORM,
    requirement: 'Content H2s open by advancing the previous claim.',
    promptInstruction: 'Open with a consequence, constraint, or next decision from the previous section. Do not re-introduce the primary keyword in the first sentence.',
    evidence: 'proseGeometry.evaluateProseGeometry via critiqueCohesion kit_section_opener',
    shipEffect: 'allow_with_flag', evaluator: 'contentQualityGate.evaluateContentQuality',
    testFixture: 'tests/prose-geometry.test.ts#kit section openers',
  }),
  def({
    code: 'stuffed_primary_opener_severe', title: 'Multiple H2s open as standalone kit pieces',
    severity: 'blocker', owner: 'writer', repairClass: 'targeted_ai', appliesTo: INDEXABLE_FORM,
    requirement: 'At most one content H2 may open by restating the primary. Two or more kit-piece openings are a ship hold.',
    promptInstruction: 'Rewrite as ONE article: each H2 must continue the previous H2. Open with a consequence, constraint, or next decision — do not re-introduce the primary keyword in the first sentence.',
    evidence: 'proseGeometry.evaluateProseGeometry kit_section_opener count ≥ 2',
    shipEffect: 'block', evaluator: 'contentQualityGate.evaluateContentQuality',
    testFixture: 'tests/prose-geometry.test.ts#kit section openers',
  }),
  def({
    code: 'adjacent_section_overlap', title: 'Adjacent H2s repeat the same argument',
    severity: 'warning', owner: 'writer', repairClass: 'targeted_ai', appliesTo: INDEXABLE_FORM,
    requirement: 'Consecutive content H2s must advance the argument (token overlap under 50%).',
    promptInstruction: 'Rewrite the later section so it starts from a new claim or next step. Do not restate the previous H2.',
    evidence: 'proseGeometry.evaluateProseGeometry via critiqueCohesion adjacent Jaccard',
    shipEffect: 'allow_with_flag', evaluator: 'contentQualityGate.evaluateContentQuality',
    testFixture: 'tests/prose-geometry.test.ts',
  }),
  def({
    code: 'adjacent_section_overlap_severe', title: 'Adjacent H2s are mill-glued copies',
    severity: 'blocker', owner: 'writer', repairClass: 'targeted_ai', appliesTo: INDEXABLE_FORM,
    requirement: 'Adjacent content H2s must not share ≥65% of content tokens.',
    promptInstruction: 'Rewrite the later section so it advances a new claim, step, or constraint. Do not restate the previous H2.',
    evidence: 'proseGeometry.evaluateProseGeometry Jaccard ≥ 0.65',
    shipEffect: 'block', evaluator: 'contentQualityGate.evaluateContentQuality',
    testFixture: 'tests/prose-geometry.test.ts',
  }),
  def({
    code: 'faq_duplicates_h2', title: 'FAQ question restates an H2',
    severity: 'warning', owner: 'writer', repairClass: 'targeted_ai', appliesTo: INDEXABLE_FORM,
    requirement: 'FAQ questions must not paste a content H2 heading.',
    promptInstruction: 'Ask a reader question the H2 did not already answer. Do not paste the heading into the FAQ.',
    evidence: 'proseGeometry.evaluateProseGeometry via critiqueCohesion',
    shipEffect: 'allow_with_flag', evaluator: 'contentQualityGate.evaluateContentQuality',
    testFixture: 'tests/prose-geometry.test.ts',
  }),
  def({
    code: 'repeated_paragraph_opener', title: 'Same first four words open ≥3 paragraphs',
    severity: 'warning', owner: 'writer', repairClass: 'targeted_ai', appliesTo: INDEXABLE_FORM,
    requirement: 'Paragraph openings vary; the same first four words appear in fewer than 3 paragraphs.',
    promptInstruction: 'Vary paragraph openings. Lead with a new actor, constraint, or next step.',
    evidence: 'proseGeometry.evaluateProseGeometry via critiqueCohesion',
    shipEffect: 'allow_with_flag', evaluator: 'contentQualityGate.evaluateContentQuality',
    testFixture: 'tests/prose-geometry.test.ts',
  }),
  def({
    code: 'low_sentence_burstiness', title: 'Sentence length too uniform',
    severity: 'warning', owner: 'writer', repairClass: 'targeted_ai', appliesTo: INDEXABLE_FORM,
    requirement: 'Sentence-length coefficient of variation stays above 0.28 on blogs and 0.16 on guides.',
    promptInstruction: 'Follow a longer explanatory sentence with a short one. Do not write a page of 14-word sentences.',
    evidence: 'proseGeometry.evaluateProseGeometry sentence-length CV',
    shipEffect: 'allow_with_flag', evaluator: 'contentQualityGate.evaluateContentQuality',
    testFixture: 'tests/prose-geometry.test.ts',
  }),
  def({
    code: 'low_trigram_variety', title: 'Looping mill phrases',
    severity: 'warning', owner: 'writer', repairClass: 'targeted_ai', appliesTo: INDEXABLE_FORM,
    requirement: 'Unique 3-gram ratio stays above 0.55 on blogs and 0.45 on guides.',
    promptInstruction: 'Cut repeated three-word loops. Say the official-site reminder once, then advance.',
    evidence: 'proseGeometry.evaluateProseGeometry unique 3-gram ratio',
    shipEffect: 'allow_with_flag', evaluator: 'contentQualityGate.evaluateContentQuality',
    testFixture: 'tests/prose-geometry.test.ts',
  }),
  def({
  ...
  def({
    ...
    promptInstruction: 'Cover each missing demand short as a topic in a grammatical sentence. Meaning coverage beats exact-string placement. Never paste the phrase into the first content H2, In 60 seconds, a heading, or an FAQ question.',
  ...
  def({
    ...
    promptInstruction: 'Answer the reader question this long-tail represents in prose or an FAQ answer. Do not paste the string as an H2 or as the question text.',
    evidence: 'contentQualityGate.evaluateContentQuality keyword presence',
    shipEffect: 'block', evaluator: 'contentQualityGate.evaluateContentQuality',
    testFixture: 'tests/contentQualityPlaybook.test.ts#keyword fixtures',
  }),
  def({
    ...
    promptInstruction: 'Cover the synthesized short as a topic only if a grammatical slot already exists. Skip any that cannot be placed without force-fitting. Never paste into the first content H2 or an FAQ question.',
  ...
  def({
    ...
    promptInstruction: 'Answer the synthesized long-tail in prose only if a grammatical slot already exists. Skip any that cannot be placed without force-fitting. Never paste as an H2 or FAQ question.',
    evidence: 'contentQualityGate.evaluateContentQuality keyword presence (provenance: synthesized)',
    shipEffect: 'allow_with_flag', evaluator: 'contentQualityGate.evaluateContentQuality',
    testFixture: 'tests/keyword-coverage.test.ts#warns instead of blocking when synthesized backfill terms are uncovered',
  }),
  ...
  def({
    code: 'missing_outline_section', title: 'Brief outline section missing from the body',
    severity: 'blocker', owner: 'writer', repairClass: 'targeted_ai', appliesTo: INDEXABLE_FORM,
    requirement: 'Every non-structural section in the canonical brief outline must exist in the article. A truncated draft that clears other floors ("the next section walks through…") is a structural defect and must not ship.',
    promptInstruction: 'Write the missing section(s) named in the finding as natural prose for the reader — each must satisfy its outline purpose. Insert before the FAQ (or at the end). Never paste keyword strings. Executor is the outline-completion hop (completeMissingOutlineSections), never Harper — Harper cannot add H2s.',
    evidence: 'contentQualityGate.missingOutlineSections (canonical outline vs body headings)',
    shipEffect: 'block', evaluator: 'contentQualityGate.evaluateContentQuality',
    testFixture: 'tests/content-quality.test.ts#missing_outline_section',
  }),
  def({
    code: 'forward_reference_orphan', title: 'Text promises a section that does not exist',
    severity: 'warning', owner: 'writer', repairClass: 'deterministic', appliesTo: INDEXABLE_FORM,
    requirement: 'No "the next section walks through… / as shown below" connector may reference a section that never appears.',
    promptInstruction: 'Remove the dangling connector sentence, or write the promised section within this article.',
    evidence: 'contentQualityGate.detectDanglingForwardReferences',
    shipEffect: 'allow_with_flag', evaluator: 'contentQualityGate.evaluateContentQuality',
  ...
  def({
    ...
    severity: 'warning', owner: 'deterministic', repairClass: 'deterministic', appliesTo: INDEXABLE_FORM,
    requirement: 'Headings name the section for a reader; they are never the brief\'s keyword strings verbatim ("### Requirements for a study abroad consultant cost"). Audit & Fix rewrites them deterministically via applyDeterministicRepairs (keyword_pasted_headings_rewritten).',
    ...
    evidence: 'contentQualityGate.evaluateContentQuality heading-vs-keyword scan + editorialScaffold.rewritePastedHeading',
    shipEffect: 'allow_with_flag', evaluator: 'contentQualityGate.evaluateContentQuality',
    testFixture: 'tests/editorial-scaffold.test.ts#keyword-pasted heading deterministic rewrite',
  }),
  def({
  ...
  def({
    ...
    promptInstruction: 'Change keyword, content type, or expand the existing owner URL. Never Harper — owner URL is frozen identity. Re-run Discover or confirm with the operator; silent rewrite of a canonical is worse than a hold.',
  ...
  def({
    ...
    promptInstruction: 'Include the primary keyword naturally in title and first H2. Title/H1 identity is frozen at briefing — Harper cannot change the primary or the H1; Audit & Fix / title lab owns heading placement. Harper may only echo the existing primary in opening prose.',
  ...
  def({
    code: 'ahrefs_canonical_off_estate', title: 'Off-estate canonical (Ahrefs)',
    severity: 'blocker', owner: 'deterministic', repairClass: 'deterministic', appliesTo: 'all',
    requirement: 'Canonical URL must be on the YouSafe estate (ca/uk/usa/au/legal/apex), not a government or citation host.',
    promptInstruction: 'Set canonicalUrl to the YouSafe owner-plan URL. Repair overwrites non-estate values from targetUrl.',
    evidence: 'ahrefsIssues.evaluateAhrefsDraft isEstateCanonicalUrl',
    shipEffect: 'block', evaluator: 'ahrefsIssues.evaluateAhrefsDraft',
    testFixture: 'tests/contentQualityPlaybook.test.ts#ahrefs fixtures',
  }),
  def({
    code: 'ahrefs_canonical_mismatch', title: 'Canonical mismatch vs owner target (Ahrefs)',
    severity: 'warning', owner: 'deterministic', repairClass: 'deterministic', appliesTo: 'all',
    requirement: 'Front-matter canonicalUrl matches the owner-plan live URL when both are on-estate.',
    promptInstruction: 'Align canonicalUrl with the owner-plan targetUrl before ship.',
    evidence: 'ahrefsIssues.evaluateAhrefsDraft canonical vs targetUrl',
    shipEffect: 'allow_with_flag', evaluator: 'ahrefsIssues.evaluateAhrefsDraft',
    testFixture: 'tests/contentQualityPlaybook.test.ts#ahrefs fixtures',
  }),
  def({
