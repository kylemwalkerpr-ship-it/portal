...
function renderConsultancyBlogPage(opts: {
  ...
  // Extract the intro paragraph (text before the first H2) for the header AND
  // slice it out of the body so it never renders twice.
  const firstH2 = body.search(/\n##\s/)
  const introRaw = (firstH2 >= 0 ? body.slice(0, firstH2) : body).trim()
  const intro =
    introRaw.length >= 60 ? renderInline(introRaw.replace(/\s+/g, ' ')) : renderInline(description)
  const jsxBody = markdownToBlogJsx(firstH2 >= 0 ? body.slice(firstH2) : '')
