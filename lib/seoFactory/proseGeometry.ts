 ...
 * Severe adjacent-section overlap is the only automatic mill-glue ship hold.
 * Two or more kit-section openings also hold — that is a stitched kit, not an article.
 */
export function evaluateProseGeometry(
  ...
  const kitOpeners: Array<{ message: string; evidence?: string }> = []
  for (const finding of cohesion.findings) {
    ...
    if (finding.code === 'kit_section_opener') {
      kitOpeners.push(finding)
    }
  }
