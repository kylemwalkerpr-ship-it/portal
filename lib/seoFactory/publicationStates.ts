...
export function evaluateLiveArtifact(input: {
  ...
  if (input.hasNoIndex) return { ok: false, phase: 'verification_failed', reason: 'noindex on live document' }
  if (!input.canonicalMatches) return { ok: false, phase: 'verification_failed', reason: 'canonical mismatch' }
