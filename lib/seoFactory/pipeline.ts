...
export async function runSeoFactoryPipeline(input: PipelineInput): Promise<PipelineResult> {
  ...
  // FAIL-CLOSED automatic interlinks (P6 parity with pipelineStream). Every
  // automatic input/planner interlink is pruned by the live internal-URL
  // authority, and every marketplace provider/profile/gig citation link is
  // verified by the repository's ACTUAL HTTP liveness authority
  // (`verifyMarketplaceServiceUrlsLive`) — the estate helper's
  ...
  let automaticInterlinks: Array<{ label?: string; url?: string; matchedOn?: string[] }> = []
  {
    ...
    automaticInterlinks = [...pruned.links, ...providerPruned.links].filter((link) => {
      const key = String(link?.url || '').replace(/\/+$/, '').toLowerCase()
  ...
  // prompts and persisted in the job audit payload. Fails closed and safe:
  // an invalid/unresolvable spec keeps the pre-spec behavior (null spec)
  // instead of weakening the run.
  let contentSpec: ContentSpec | null = null
  try {
    const specResolution = resolveContentSpecForJob({
      jobId: input.existingJobId || input.sourceJobId || `plan-${Date.now()}`,
      contentType,
      region,
    ...
    console.warn('[seoFactory/pipeline] ContentSpec resolution skipped', specErr)
    contentSpec = null
  }
  ...
  const system = buildFactorySystemPrompt({
    ...
    interlinkAllowlist: automaticInterlinks,
    ...
    try {
      const linear = await runLinearDesk({
        ...
        assembly: assemblyFromPipelineInput({
          ...
          lede: input.lede,
          sources: verifiedSources,
          interlinks: automaticInterlinks,
          requiredShortKeywords,
          requiredLongTailKeywords,
  ...
  for (let i = 0; i <= maxRefine; i++) {
    ...
    if (i === 0 && linearDrafted && content) {
      ...
      const q = evaluateContentQuality({
        ...
        longTailKeywordTerms,
        region,
        linkAllowlist: automaticInterlinks.map((l) => l.url).filter(Boolean) as string[],
      })
      refineNotes = [
  ...
  if (!deskHeld) {
    ...
    if (denoise.applied) {
      ...
      audit = runAudit(content)
      console.info(`[seoFactory/pipeline] masked denoise applied passes=${denoise.passes} spans=${denoise.spans}`)
    } else if (denoise.rejected && denoise.reason) {
      console.warn('[seoFactory/pipeline] masked denoise rejected:', denoise.reason)
    }
  }
  ...
  if (shipMode !== 'none' && !shipError) {
    ...
    shipResult = await shipContent({
      ...
      // EXACT persisted content_jobs.id, when the caller really has one. The
      // non-stream path persists its job row AFTER ship, so there is no early
      // row to bind instead — and a synthetic `plan-*` id must never be
      // passed as job identity. No existing job = no jobId (jobless staging,
      // never auto-finalized by the scheduled reconciler).
      ...(input.existingJobId ? { jobId: input.existingJobId } : {}),
      requiredShortKeywords,
      requiredLongTailKeywords,
      shortKeywordTerms,
      longTailKeywordTerms,
