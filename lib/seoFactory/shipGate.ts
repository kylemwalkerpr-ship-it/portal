...
export function validateRenderedPayload(opts: {
  ...
  if (plan.repo === 'caseworks' || (filePath.endsWith('page.tsx') && plan.host === 'legal')) {
    ...
    for (const block of ctaBlocks) {
      ...
      // href must be a non-empty string literal or template — never bare expression
      const hrefStr = block.match(/\bhref\s*=\s*\{?\s*(["'`])([^"'`]*)\1/)
      const hrefJson = block.match(/\bhref\s*=\s*(\{JSON\.stringify\([^)]+\)\})/)
      if (hasHref && !hrefStr && !hrefJson && !/\bhref\s*=\s*\{["'`]/.test(block) && !/\bhref\s*=\s*["'`]/.test(block)) {
        // Allow href={"/intake?..."} form
        if (!/\bhref\s*=\s*\{\s*["'`][^"'`]+["'`]\s*\}/.test(block)) {
          errors.push('CTAPanel href must be a non-empty string (build-safe Link contract)')
        }
      }
      if (hrefStr && !hrefStr[2].trim()) {
        errors.push('CTAPanel href is empty string — refuse ship')
      }
    }
    ...
    // Syntax hazards that break Next static export prerender
    ...
    if (/```/.test(content)) {
      errors.push('caseworks page.tsx still contains markdown fences — refuse ship')
    }
    ...
    // country must be a known ArticleMeta union member
    const countryM = content.match(/country:\s*["'](\w+)["']/)
