 ...
 * store / download / official / services / contact details / company suffix /
 * country-city suffixes), plus an optional TRAILING calendar year. BOTH
 ...
 * `you safe consultancy london`, and `you safe contact number`,
 * `you safe ltd` / `you safe limited` and `you safe reviews 2024` are the same
 * genre of self-search. It is never an unbounded `^you safe\b.*`: ordinary
 * prose that merely contains or starts with the words ("you safe to travel on
 * a student visa", "are you safe to travel on a student visa", "is warwick
 * safe for international students", "you safe phone number for international
 * students") is real search demand and must never be junked by the brand rule.
 */
const SELF_BRAND_NAV_SRC =
  '(?:log ?in|sign ?in|reviews?|apps?|downloads?|portals?|contacts?|websites?|sites?|pages?|stores?|homepages?|official|services?|numbers?|phones?|e ?mails?|address(?:es)?|opening\\s+hours?|hours?|uk|england|scotland|wales|london|ltd|limited)'

/**
 * Trailing calendar year on a navigational self-search (`you safe reviews
 * 2024`). The year is a modifier of the WHOLE branded self-search, so it stays
 * outside the two qualifier slots — it can never open a third free word.
 */
const SELF_BRAND_YEAR_SRC = '(?:19|20)\\d{2}'

const SPACED_SELF_BRAND_RE = new RegExp(
  `^you safe(?: consultancy)?(?: ${SELF_BRAND_NAV_SRC})?(?: ${SELF_BRAND_NAV_SRC})?(?: ${SELF_BRAND_YEAR_SRC})?$`,
)
