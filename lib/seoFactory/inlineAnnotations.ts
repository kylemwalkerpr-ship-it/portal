...
export function buildBlockersFixPrompt(
  ...
  return [
    ...
    '   c) No section heading may appear more than once.',
    '   d) NO padding: do NOT repeat content to fill space. Write NEW content instead.',
    '5. External-link blockers — ONE pass, do not iterate:',
    '   - READ the sentence around each flagged URL.',
    '   - KEEP the href if that host is the issuing body for the claim (the board, council, or exam administrator named in the sentence). Do not swap a board/exam URL for USCIS or a generic .gov homepage.',
    '   - If the host is a blog, news, Wikipedia, competitor, or shortener: REPLACE the href in place with the allowlist official URL that supports the SAME claim. Keep the anchor text.',
    '   - Only unwrap (delete the href, keep the words) when no live official URL fits. Then add one official citation next to the claim or under ## Official sources.',
    '   - Never invent a URL. Never leave a 404. Never replace every external link with the same immigration homepage.',
    '6. Return the COMPLETE article only.',
    '',
    'CURRENT ARTICLE:',
...
export function buildWarningsFixPrompt(
  ...
  return [
    ...
    '   a clearly labeled example, or address the reader directly — where it fits.',    '4. REPETITION ELIMINATION — do NOT repeat any sentence, paragraph, or phrase:',
    '   a) No sentence may appear more than once (even reworded). DELETE the weaker copy.',
    '   b) No paragraph may be duplicated or nearly-duplicated (≥80% same words). MERGE duplicates.',
    '   c) No section heading may appear more than once.',
    '   d) NO padding: do NOT repeat content to fill space. Write NEW substantive content instead.',
    '5. KEYWORD_DENSITY_HIGH — if this warning is listed, the primary keyword appears too often:',
    '   a) Find EVERY occurrence of the exact primary keyword phrase in the article.',
    '   b) KEEP the first 2–3 occurrences (they anchor SEO signals in intro + first H2).',
    '   c) REPLACE the remaining occurrences with natural paraphrases:',
    '      - "UK dependent visa documents checklist" → "this guide", "the process",',
    '        "these requirements", "this document", "the application", "this procedure".',
    '      - "MBA essay editing service" → "this service", "the editing process",',
    '        "these providers", "the application support".',
    '      - For any keyword: split into parts and use synonyms for the repeated word.',
    '   d) Target: reduce to ~2–3% density (the original 4%+ is flagged as stuffing).',
    '   e) NEVER remove the keyword from the title, H1, meta description, or first paragraph.',
    '6. Do not pad with repetition. Fix, then stop.',
