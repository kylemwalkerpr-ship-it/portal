 ...
 * Commissioned Content Studio providers only: xAI (Grok) and first-party
 * DeepSeek (api.deepseek.com).
 */
export type Provider='xai'|'deepseek'
...
export function breakerStatus(){ const out: Array<{provider:Provider, fails:number, open:boolean, retryInSec:number}> =[]; for(const p of ['xai','deepseek'] as Provider[]){ const cur=fails.get(p); const open=isOpen(p); out.push({provider:p, fails:cur?.count??0, open, retryInSec: open? Math.ceil((WINDOW_MS-(Date.now()-cur!.lastAt))/1000):0 }) } return out }
