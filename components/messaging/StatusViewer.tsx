...
export default function StatusViewer({ statuses, initialIndex = 0, onClose, viewerId, viewerRole, onRespond, onOpenProfile, onDeleteBroadcast }: StatusViewerProps) {
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Brief 47 §6.3: Delete broadcast control for student's own status */}
        {isMine && onDeleteBroadcast && status.inquiry_id && (
          <div style={{ padding: '0 16px 16px', display: 'flex', justifyContent: 'center' }}>
            <button
              onClick={() => onDeleteBroadcast(status.id, status.inquiry_id!)}
              style={{
