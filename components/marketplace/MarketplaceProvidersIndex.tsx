...
import { C, Btn, Card, Badge } from '../design/shared'
...
const NAVY='#1B2D4F', GOLD='#9A7B3B', CYAN='#0E7C8E', PURPLE='#3D2B6B', AMBER='#8B5E0A', GREEN='#1A6B45', RED='#8B1A1A'
    ...
    <div style={{ minHeight: '100vh', background: BG, fontFamily: SANS, color: TEXT }}>
      <div style={{ width: 'min(1280px, calc(100vw - 32px))', margin: '0 auto', padding: '32px 0 64px' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', flexWrap: 'wrap', gap: 12, marginBottom: 22 }}>
          <div>
            <div style={{ fontSize: 11, fontWeight: 700, letterSpacing: '.16em', color: GOLD, textTransform: 'uppercase', fontFamily: MONO, marginBottom: 4 }}>Verified panel</div>
            <h1 style={{ fontFamily: SERIF, fontSize: 36, fontWeight: 500, color: TEXT, margin: 0, letterSpacing: '-.012em' }}>All providers.</h1>
        ...
        <div className="yousafe-mobile-stack" style={{ display: 'grid', gridTemplateColumns: showFilters ? '280px 1fr' : '1fr', gap: 18 }}>
          ...
          {showFilters && (
            <aside style={{ background: SURFACE, border: `1px solid ${BORDER}`, borderRadius: 10, padding: 16, display: 'flex', flexDirection: 'column', gap: 16, alignSelf: 'flex-start', position: 'sticky', top: 18 }}>
              ...
              <div>
                <div style={{ fontSize: 10, fontWeight: 700, letterSpacing: '.12em', textTransform: 'uppercase', color: DIM, fontFamily: MONO, marginBottom: 6 }}>Sort</div>
          ...
          <main style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            ...
            {!loading && !error && attorneys.length === 0 && (
              <Card style={{ padding: 40, textAlign: 'center' }}>
                ...
                <div style={{ fontFamily: SERIF, fontSize: 22, fontWeight: 600, color: TEXT, marginBottom: 4 }}>No providers match these filters</div>
    ...
    <Link href={`/sellers/${a.id}`} style={{ textDecoration: 'none', display: 'block' }}>
      <Card
        ...
        <div style={{ display: 'flex', gap: 12, alignItems: 'center' }}>
          {a.headshot_url
            ? <img src={a.headshot_url} alt="" style={{ width: 56, height: 56, borderRadius: '50%', objectFit: 'cover', border: `1px solid ${BORDER}` }} />
            : <div style={{ width: 56, height: 56, borderRadius: '50%', background: `${NAVY}10`, color: NAVY, display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 700, fontSize: 18, fontFamily: SERIF }}>{initials(a.full_name)}</div>}
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ fontFamily: SERIF, fontWeight: 600, fontSize: 18, color: TEXT, lineHeight: 1.15, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{a.full_name}</div>
            {a.rating_avg !== null
              ? (
                <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginTop: 2 }}>
                  <Stars value={a.rating_avg} />
                  <span style={{ fontSize: 12, color: TEXT, fontWeight: 700, fontFamily: MONO }}>{a.rating_avg}</span>
                  <span style={{ fontSize: 11, color: DIM, fontFamily: MONO }}>({a.rating_count})</span>
                </div>
              )
          ...
          <div>
            {a.starting_price > 0 && (
              <>
                ...
                <div style={{ fontFamily: SERIF, fontSize: 18, fontWeight: 600, color: TEXT, lineHeight: 1 }}>{fmtPrice(a.starting_price)}</div>
              </>
            )}
          </div>
