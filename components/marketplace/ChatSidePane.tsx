...
function normalizeUnifiedThread(payload: any) {
  ...
  return (Array.isArray(payload?.messages) ? payload.messages : []).map((m: any) => ({
    ...
    ref_offer_id: m.ref_offer_id || null,
    ref_order_id: m.ref_order_id || null,
    ref_inquiry_id: m.ref_inquiry_id || null,
    offer: m.offer || null,
...
function AiMessageBody({ body }: { body: React.ReactNode }) {
  return (
    <span className="ys-ai-message">
      <span className="ys-ai-message-label">✦ YQAA</span>
...
export default function ChatSidePane({
  ...
  const [offerBusyId, setOfferBusyId] = React.useState<string | null>(null)
  ...
  const loadLegacyChat = React.useCallback(async () => {
    ...
    try {
      ...
      if (r.status === 401) {
        setError('SIGN_IN_REQUIRED'); setChatId(null); setMessages([]); return
      }
      const d = await r.json().catch(() => ({}))
      if (r.ok) {
        const list = d?.chats || []
  ...
  React.useEffect(() => { if (open) void loadLegacyChat() }, [open, loadLegacyChat])
  ...
  const loadUnifiedConversation = React.useCallback(async (id: string, silent = false) => {
    ...
    try {
      const r = await fetch(`/api/messages/conversations/${id}`, { credentials: 'include', cache: 'no-store' })
      const d = await r.json().catch(() => ({}))
      if (seq !== unifiedSeqRef.current) return
      if (!r.ok) {
        if (r.status === 401) { setError('SIGN_IN_REQUIRED'); return }
  ...
  React.useEffect(() => {
    if (!open) return
    const refresh = () => {
      if (document.visibilityState === 'visible' && conversationIdRef.current) void loadUnifiedConversation(conversationIdRef.current, true)
    }
      ...
      if (attorneyId) {
        if (!chatId) {
          const r = await fetch('/api/client/attorney-message', {
            method: 'POST', credentials: 'include', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ attorneyId, message: text }),
          })
          const d = await r.json().catch(() => ({}))
          if (!r.ok || !d?.chatId) throw new Error(d?.error || 'Could not start chat.')
          setChatId(d.chatId)
          if (d.conversationId) { setConversationId(d.conversationId); conversationIdRef.current = d.conversationId }
          setDraft('')
  ...
  const handleOfferAccept = React.useCallback((offerId: string) => {
    if (!offerId || offerBusyId) return
    setError('')
    setPayingOfferId(offerId)
  }, [offerBusyId])

  const handleOfferDecline = React.useCallback(async (offerId: string) => {
    if (!offerId || offerBusyId) return
    setOfferBusyId(offerId); setError('')
    try {
      const r = await fetch(`/api/offers/${offerId}/decline`, {
        method: 'PATCH', credentials: 'same-origin', headers: { 'Content-Type': 'application/json' },
      })
      const d = await r.json().catch(() => ({}))
      if (!r.ok) throw new Error(d?.error?.message || d?.error || `Failed (${r.status})`)
      if (conversationIdRef.current) await loadUnifiedConversation(conversationIdRef.current, true)
    } catch (e: any) {
      setError(e?.message || 'Could not decline offer.')
    } finally {
      setOfferBusyId(null)
    }
  }, [offerBusyId, loadUnifiedConversation])
  ...
  const aiLiveBanner = conversationId && aiMode === 'auto' ? (
    <div className="ys-market-ai-live" role="status" aria-live="polite">
      ...
      <span><strong>YQAA live replies are on.</strong> The specialist can join at any time.</span>
  ...
  const errorBanner = error === 'SIGN_IN_REQUIRED' ? (
    <div className="ys-market-chat-signin" style={{ padding: '12px 14px', background: `${CYAN}10`, color: CYAN, fontSize: 12, fontWeight: 600, display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 10 }}>
      ...
      <a href={`https://portal.yousafeconsultancy.com/sign-in/student?return_to=${encodeURIComponent(typeof window !== 'undefined' ? window.location.href : '/')}`} style={{ background: CYAN, color: '#FFFFFF', padding: '8px 12px', borderRadius: 999, textDecoration: 'none', fontWeight: 700, whiteSpace: 'nowrap' }}>Sign in →</a>
    </div>
  ) : error ? (
    <div style={{ padding: '10px 14px', background: `${RED}10`, color: RED, fontSize: 12, fontWeight: 600 }}>{error}</div>
  ) : null

  const banner = <>{aiLiveBanner}{errorBanner}</>
  ...
  const messageNodes = React.useMemo(() => {
    ...
    if (!loading && messages.length === 0) {
      result.push(
        <div key="empty" className="ys-market-chat-empty" style={{ background: SURFACE, border: `1px dashed ${BORDER}`, borderRadius: 10, padding: '20px 16px', textAlign: 'center' }}>
          ...
          <div style={{ fontSize: 12, color: MUTED, lineHeight: 1.5 }}>Ask a short question. When live replies are enabled, YQAA can respond while {attorneyName || 'the specialist'} is away, and the specialist can take over at any time.</div>
        </div>,
      )
      return result
    }
      ...
      if (showDate) {
        result.push(
          <div key={`date-${m.id}`} style={{ display: 'flex', justifyContent: 'center', margin: '10px 0' }}>
            <span style={{ fontSize: 11, fontWeight: 600, color: MUTED, background: 'rgba(0,0,0,0.06)', padding: '4px 12px', borderRadius: 999, letterSpacing: '.02em' }}>{dateLabel(m.created_at)}</span>
          </div>,
        )
      }

      if (m.type === 'offer' && m.offer?.id) {
        result.push(
          <div key={m.id} className="bubrow theirs last" data-msgid={m.id} style={{ display: 'flex', justifyContent: 'flex-start' }}>
            <MessageOfferCard
              offer={m.offer}
              viewerRole="buyer"
              ...
              onOpenOrder={(orderId) => { window.location.href = marketplaceOrdersHref(orderId) }}
            />
          </div>,
        )
        continue
    ...
    return result
  }, [messages, loading, attorneyName, offerBusyId, handleOfferAccept, handleOfferDecline])
  ...
  const composer = (
    <div className="ys-market-chat-composer" style={{ borderTop: `1px solid ${BORDER}`, background: SURFACE }}>
      <AutoGrowInput
        ...
        onAttachmentSent={() => { if (conversationIdRef.current) void loadUnifiedConversation(conversationIdRef.current, true) }}
      />
      <div className="ys-market-chat-foot" style={{ padding: '0 14px 10px', fontSize: 10, color: DIM, fontFamily: MONO, display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 8 }}>
        <span className="ys-market-chat-shortcuts"><kbd style={{ padding: '1px 5px', background: BG, border: `1px solid ${BORDER}`, borderRadius: 3, fontFamily: MONO, fontSize: 9 }}>Enter</kbd> send · <kbd style={{ padding: '1px 5px', background: BG, border: `1px solid ${BORDER}`, borderRadius: 3, fontFamily: MONO, fontSize: 9 }}>Esc</kbd> close</span>
        {conversationId && <a href={`https://portal.yousafeconsultancy.com/dashboard?page=messages&thread=${conversationId}`} style={{ color: CYAN, fontWeight: 700, fontFamily: SANS, fontSize: 11, textDecoration: 'none', whiteSpace: 'nowrap' }}>Open in Messages →</a>}
      </div>
    </div>
  )
  ...
  return (
    <>
      <div className="ys-market-chat-overlay" style={{ position: 'fixed', inset: 0, zIndex: 200, display: 'flex', justifyContent: 'flex-end' }}>
        ...
        <aside className="yousafe-messenger chat-side-pane" data-theme="light" style={{ width: 'min(440px, 100vw)', height: '100dvh', background: 'var(--bg, #F7F8FA)', display: 'flex', flexDirection: 'column', borderLeft: `1px solid ${BORDER}`, boxShadow: '-24px 0 60px rgba(29,36,51,0.18)', fontFamily: SANS, color: TEXT, colorScheme: 'light', isolation: 'isolate' }}>
