          ...
          <ul style={{ margin: '10px 0 0', padding: '12px 0 0', borderTop: '1px solid #F1F5F9', listStyle: 'none', display: 'flex', flexDirection: 'column', gap: 6 }}>
            {missing.slice(0, 8).map((c) => (
              <li key={c.id} style={{ fontSize: 13, color: '#334155', display: 'flex', gap: 8, alignItems: 'baseline' }}>
                ...
                <span><b style={{ color: '#0F172A' }}>{c.label}</b>{c.hint ? <span style={{ color: '#64748B' }}> — {c.hint}</span> : null}{' '}
                  <span style={{ fontFamily: "'IBM Plex Mono', monospace", fontSize: 11, color: '#64748B' }}>+{c.weight}%</span></span>
              </li>
            ))}
          </ul>
        )}
...
const pageWrap: React.CSSProperties = { minHeight: '100vh', background: '#F7F8FA', fontFamily: "var(--font-inter), Inter, system-ui, sans-serif", color: '#0F172A', padding: '24px 16px 60px' }
const headerStyle: React.CSSProperties = { width: 'min(900px, calc(100vw - 32px))', margin: '0 auto 18px' }
const breadStyle: React.CSSProperties = { fontFamily: "var(--font-plex-mono), 'IBM Plex Mono', monospace", fontSize: 11, letterSpacing: '0.12em', textTransform: 'uppercase', color: '#334155', textDecoration: 'none' }
const titleStyle: React.CSSProperties = { margin: '8px 0 6px', fontFamily: "var(--font-lora), Lora, Georgia, serif", fontSize: 32, fontWeight: 500, letterSpacing: '-0.018em', color: '#0F172A' }
const subtitleStyle: React.CSSProperties = { margin: 0, fontSize: 14.5, lineHeight: 1.55, color: '#334155', maxWidth: '60ch' }
const scoreRow: React.CSSProperties = { marginTop: 14, display: 'flex', alignItems: 'center', gap: 14, flexWrap: 'wrap' }
const progressTrack: React.CSSProperties = { flex: 1, minWidth: 200, height: 8, background: '#F1F5F9', borderRadius: 999, overflow: 'hidden' }
...
const stepChip = (active: boolean, done: boolean): React.CSSProperties => ({ display: 'inline-flex', alignItems: 'center', gap: 6, padding: '6px 12px', borderRadius: 999, border: `1px solid ${active ? '#0F172A' : '#E2E8F0'}`, background: active ? '#0F172A' : (done ? '#EEF1F6' : '#FFFFFF'), color: active ? '#fff' : '#0F172A', fontSize: 12, fontWeight: 600, cursor: 'pointer', whiteSpace: 'nowrap' })
const stepNum = (active: boolean, done: boolean): React.CSSProperties => ({ display: 'inline-grid', placeItems: 'center', width: 18, height: 18, borderRadius: '50%', fontSize: 10, fontWeight: 700, background: active ? '#fff' : (done ? '#1A6B3A' : '#fff'), color: active ? '#0F172A' : (done ? '#fff' : '#0F172A'), border: done ? 'none' : '1px solid #E2E8F0' })
const cardStyle: React.CSSProperties = { width: 'min(900px, calc(100vw - 32px))', margin: '0 auto', background: '#FFFFFF', border: '1px solid #E2E8F0', borderRadius: 14, padding: '22px 26px 18px', boxShadow: '0 14px 30px -22px rgba(29,36,51,0.2)' }
const stepHead: React.CSSProperties = { marginBottom: 18 }
const stepEyebrow: React.CSSProperties = { fontFamily: "var(--font-plex-mono), 'IBM Plex Mono', monospace", fontSize: 10.5, letterSpacing: '0.14em', textTransform: 'uppercase', color: '#64748B' }
const stepTitle: React.CSSProperties = { margin: '4px 0 4px', fontFamily: "var(--font-lora), Lora, serif", fontSize: 24, fontWeight: 500, letterSpacing: '-0.012em', color: '#0F172A' }
...
const inputStyle: React.CSSProperties = { width: '100%', padding: '10px 12px', border: '1px solid #E2E8F0', borderRadius: 8, background: '#FFFFFF', fontSize: 14, color: '#0F172A', fontFamily: 'inherit', boxSizing: 'border-box' }
...
const primaryBtn: React.CSSProperties = { background: '#0F172A', color: '#fff', border: 'none', padding: '10px 18px', borderRadius: 999, fontSize: 13.5, fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit' }
const ghostBtn: React.CSSProperties = { background: '#FFFFFF', color: '#0F172A', border: '1px solid #E2E8F0', padding: '9px 16px', borderRadius: 999, fontSize: 13, fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit' }
const pillBtn = (on: boolean): React.CSSProperties => ({ padding: '6px 14px', borderRadius: 999, border: `1px solid ${on ? '#0F172A' : '#E2E8F0'}`, background: on ? '#0F172A' : '#FFFFFF', color: on ? '#fff' : '#334155', fontSize: 12.5, fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit' })
const photoFrame: React.CSSProperties = { width: 96, height: 96, borderRadius: '50%', overflow: 'hidden', background: '#EEF1F6', display: 'grid', placeItems: 'center', border: '1px solid #E2E8F0' }
const uploadCta = (uploading: boolean): React.CSSProperties => ({ display: 'inline-flex', alignItems: 'center', padding: '9px 16px', background: uploading ? '#64748B' : '#3C3B6E', color: '#fff', borderRadius: 999, fontSize: 13, fontWeight: 600, cursor: uploading ? 'wait' : 'pointer' })
const summaryBox: React.CSSProperties = { background: '#EEF1F6', border: '1px solid', borderRadius: 12, padding: '14px 16px' }
const summaryEyebrow: React.CSSProperties = { fontFamily: "var(--font-plex-mono), monospace", fontSize: 10.5, letterSpacing: '0.14em', textTransform: 'uppercase', color: '#334155' }
const summaryTitle: React.CSSProperties = { margin: '4px 0 0', fontFamily: "var(--font-lora), serif", fontSize: 18, fontWeight: 500, color: '#0F172A' }
...
const notice: React.CSSProperties = { textAlign: 'center', color: '#64748B', fontSize: 14 }
const tipText: React.CSSProperties = { width: 'min(900px, calc(100vw - 32px))', margin: '14px auto 0', fontFamily: "var(--font-plex-mono), monospace", fontSize: 11, letterSpacing: '0.06em', color: '#64748B', textAlign: 'center' }
const globalCss = `
