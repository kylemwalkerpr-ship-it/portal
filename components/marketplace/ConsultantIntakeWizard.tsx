    ...
    <div style={fieldStack}>
      ...
      >
        <input
          ...
          style={inputStyle}
        />
        {raw && !valid && <p style={{ color: '#B22234', fontSize: 12, margin: '6px 0 0' }}>Doesn't match the slug pattern.</p>}
        {raw && valid && <p style={{ color: '#1A6B3A', fontSize: 12, margin: '6px 0 0' }}>Your profile will live at <b>/providers/{raw}</b>.</p>}
      </Field>
    </div>
  )
}
          ...
          <ul style={{ margin: '10px 0 0', padding: '12px 0 0', borderTop: '1px solid #E7E0CD', listStyle: 'none', display: 'flex', flexDirection: 'column', gap: 6 }}>
            {missing.slice(0, 8).map((c) => (
              <li key={c.id} style={{ fontSize: 13, color: '#4A4F5B', display: 'flex', gap: 8, alignItems: 'baseline' }}>
                ...
                <span><b style={{ color: '#1D2433' }}>{c.label}</b>{c.hint ? <span style={{ color: '#7B7B72' }}> — {c.hint}</span> : null}{' '}
                  <span style={{ fontFamily: "'IBM Plex Mono', monospace", fontSize: 11, color: '#7B7B72' }}>+{c.weight}%</span></span>
              </li>
            ))}
          </ul>
        )}
...
const pageWrap: React.CSSProperties = { minHeight: '100vh', background: '#FBFAF7', fontFamily: "var(--font-inter), Inter, system-ui, sans-serif", color: '#1D2433', padding: '24px 16px 60px' }
const headerStyle: React.CSSProperties = { width: 'min(900px, calc(100vw - 32px))', margin: '0 auto 18px' }
const breadStyle: React.CSSProperties = { fontFamily: "var(--font-plex-mono), 'IBM Plex Mono', monospace", fontSize: 11, letterSpacing: '0.12em', textTransform: 'uppercase', color: '#4A4F5B', textDecoration: 'none' }
const titleStyle: React.CSSProperties = { margin: '8px 0 6px', fontFamily: "var(--font-lora), Lora, Georgia, serif", fontSize: 32, fontWeight: 500, letterSpacing: '-0.018em', color: '#1D2433' }
const subtitleStyle: React.CSSProperties = { margin: 0, fontSize: 14.5, lineHeight: 1.55, color: '#4A4F5B', maxWidth: '60ch' }
const scoreRow: React.CSSProperties = { marginTop: 14, display: 'flex', alignItems: 'center', gap: 14, flexWrap: 'wrap' }
const progressTrack: React.CSSProperties = { flex: 1, minWidth: 200, height: 8, background: '#E7E0CD', borderRadius: 999, overflow: 'hidden' }
const progressFill: React.CSSProperties = { height: '100%', borderRadius: 999, transition: 'width .3s ease, background .3s ease' }
const scoreLabel = (ready: boolean): React.CSSProperties => ({ fontFamily: "var(--font-plex-mono), 'IBM Plex Mono', monospace", fontSize: 11.5, letterSpacing: '0.06em', textTransform: 'uppercase', color: ready ? '#1A6B3A' : '#3C3B6E', fontWeight: 600 })
const savedBadge: React.CSSProperties = { fontFamily: "var(--font-plex-mono), 'IBM Plex Mono', monospace", fontSize: 11, color: '#1A6B3A', fontWeight: 700 }
...
const stepChip = (active: boolean, done: boolean): React.CSSProperties => ({ display: 'inline-flex', alignItems: 'center', gap: 6, padding: '6px 12px', borderRadius: 999, border: `1px solid ${active ? '#1D2433' : '#D9D1BD'}`, background: active ? '#1D2433' : (done ? '#F4F0E6' : '#FFFEF9'), color: active ? '#fff' : '#1D2433', fontSize: 12, fontWeight: 600, cursor: 'pointer', whiteSpace: 'nowrap' })
const stepNum = (active: boolean, done: boolean): React.CSSProperties => ({ display: 'inline-grid', placeItems: 'center', width: 18, height: 18, borderRadius: '50%', fontSize: 10, fontWeight: 700, background: active ? '#fff' : (done ? '#1A6B3A' : '#fff'), color: active ? '#1D2433' : (done ? '#fff' : '#1D2433'), border: done ? 'none' : '1px solid #D9D1BD' })
const cardStyle: React.CSSProperties = { width: 'min(900px, calc(100vw - 32px))', margin: '0 auto', background: '#FFFEF9', border: '1px solid #D9D1BD', borderRadius: 14, padding: '22px 26px 18px', boxShadow: '0 14px 30px -22px rgba(29,36,51,0.2)' }
const stepHead: React.CSSProperties = { marginBottom: 18 }
const stepEyebrow: React.CSSProperties = { fontFamily: "var(--font-plex-mono), 'IBM Plex Mono', monospace", fontSize: 10.5, letterSpacing: '0.14em', textTransform: 'uppercase', color: '#7B7B72' }
const stepTitle: React.CSSProperties = { margin: '4px 0 4px', fontFamily: "var(--font-lora), Lora, serif", fontSize: 24, fontWeight: 500, letterSpacing: '-0.012em', color: '#1D2433' }
...
const inputStyle: React.CSSProperties = { width: '100%', padding: '10px 12px', border: '1px solid #D9D1BD', borderRadius: 8, background: '#FFFEF9', fontSize: 14, color: '#1D2433', fontFamily: 'inherit', boxSizing: 'border-box' }
const footerRow: React.CSSProperties = { marginTop: 18, paddingTop: 14, borderTop: '1px solid #E7E0CD', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 8 }
const primaryBtn: React.CSSProperties = { background: '#1D2433', color: '#fff', border: 'none', padding: '10px 18px', borderRadius: 999, fontSize: 13.5, fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit' }
const ghostBtn: React.CSSProperties = { background: '#FFFEF9', color: '#1D2433', border: '1px solid #D9D1BD', padding: '9px 16px', borderRadius: 999, fontSize: 13, fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit' }
const pillBtn = (on: boolean): React.CSSProperties => ({ padding: '6px 14px', borderRadius: 999, border: `1px solid ${on ? '#1D2433' : '#D9D1BD'}`, background: on ? '#1D2433' : '#FFFEF9', color: on ? '#fff' : '#4A4F5B', fontSize: 12.5, fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit' })
const photoFrame: React.CSSProperties = { width: 96, height: 96, borderRadius: '50%', overflow: 'hidden', background: '#F4F0E6', display: 'grid', placeItems: 'center', border: '1px solid #D9D1BD' }
const uploadCta = (uploading: boolean): React.CSSProperties => ({ display: 'inline-flex', alignItems: 'center', padding: '9px 16px', background: uploading ? '#7B7B72' : '#3C3B6E', color: '#fff', borderRadius: 999, fontSize: 13, fontWeight: 600, cursor: uploading ? 'wait' : 'pointer' })
const summaryBox: React.CSSProperties = { background: '#F4F0E6', border: '1px solid', borderRadius: 12, padding: '14px 16px' }
const summaryEyebrow: React.CSSProperties = { fontFamily: "var(--font-plex-mono), monospace", fontSize: 10.5, letterSpacing: '0.14em', textTransform: 'uppercase', color: '#4A4F5B' }
const summaryTitle: React.CSSProperties = { margin: '4px 0 0', fontFamily: "var(--font-lora), serif", fontSize: 18, fontWeight: 500, color: '#1D2433' }
...
const notice: React.CSSProperties = { textAlign: 'center', color: '#7B7B72', fontSize: 14 }
const tipText: React.CSSProperties = { width: 'min(900px, calc(100vw - 32px))', margin: '14px auto 0', fontFamily: "var(--font-plex-mono), monospace", fontSize: 11, letterSpacing: '0.06em', color: '#7B7B72', textAlign: 'center' }
const globalCss = `
