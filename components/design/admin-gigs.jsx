...
const sans  = C.sans
const NAVY = 'var(--portal-ink)', GOLD = 'var(--portal-gold)', GREEN = '#1A6B45'
...
const STATUS_CFG = {
  ...
  deleted:        { bg:'var(--portal-rule-soft)', text:'var(--portal-ink-soft)', border:'rgba(0,0,0,.10)',  dot:'var(--portal-ink-soft)', label:'Deleted'          },
...
const SELLER_LEVEL_CFG = {
  new_seller:  { label:'New',       color:'var(--portal-ink-soft)', bg:'var(--portal-rule-soft)' },
...
const ScoreBar = ({score,compact=false}) => { const pct=Math.min(100,Math.max(0,Number(score)||0)); const color=pct>=75?GREEN:pct>=50?AMBER:RED; if(compact) return <div style={{display:'flex',alignItems:'center',gap:'5px'}}><div style={{width:'40px',height:'4px',borderRadius:'2px',background:'var(--portal-rule-soft)',overflow:'hidden'}}><div style={{height:'100%',width:`${pct}%`,background:color,borderRadius:'2px'}}/></div><span style={{fontSize:'11px',color:'var(--portal-ink-soft)'}}>{pct}%</span></div>; return null }
    ...
    <div onClick={onClose} style={{position:'fixed',inset:0,background:'rgba(0,0,0,.60)',zIndex:320,display:'flex',alignItems:'center',justifyContent:'center',padding:'24px'}}>
      <div onClick={e=>e.stopPropagation()} style={{background:'#fff',borderRadius:'10px',padding:'28px',width:'100%',maxWidth:'480px',boxShadow:'0 20px 60px rgba(0,0,0,.22)',fontFamily:sans}}>
        ...
        <label style={{display:'block',marginBottom:'12px'}}>
          <div style={{fontSize:'11px',fontWeight:700,color:'var(--portal-ink-mid)',textTransform:'uppercase',letterSpacing:'.05em',marginBottom:'5px'}}>Category <span style={{color:RED}}>*</span></div>
          <select value={category} onChange={e=>setCategory(e.target.value)} style={{width:'100%',padding:'9px 12px',borderRadius:'7px',border:'1px solid var(--portal-rule)',fontSize:'13px',fontFamily:sans,background:'#fff'}}>
            {DENIAL_CATEGORIES.map(c=><option key={c.value} value={c.value}>{c.label}</option>)}
          </select>
        </label>
        <label style={{display:'block',marginBottom:'16px'}}>
          <div style={{fontSize:'11px',fontWeight:700,color:'var(--portal-ink-mid)',textTransform:'uppercase',letterSpacing:'.05em',marginBottom:'5px'}}>Reason for provider <span style={{color:RED}}>*</span></div>
          <textarea rows={3} value={reason} onChange={e=>setReason(e.target.value)} placeholder="Explain clearly what needs to be fixed…" style={{width:'100%',boxSizing:'border-box',border:'1px solid var(--portal-rule)',borderRadius:'7px',padding:'10px 12px',fontSize:'13px',fontFamily:sans,resize:'vertical',outline:'none',lineHeight:1.55}}/>
        </label>
        <div style={{display:'flex',gap:'8px',justifyContent:'flex-end'}}>
          <button onClick={onClose} style={{padding:'8px 18px',borderRadius:'6px',border:'1px solid var(--portal-rule)',background:'#fff',color:'var(--portal-ink-mid)',cursor:'pointer',fontSize:'13px',fontWeight:600,fontFamily:sans}}>Cancel</button>
          <button disabled={!reason.trim()||busy} onClick={async()=>{setBusy(true);await onConfirm(gig.id,reason.trim(),category);setBusy(false)}} style={{padding:'8px 18px',borderRadius:'6px',border:'none',background:!reason.trim()?'var(--portal-ink-soft)':AMBER,color:'#fff',cursor:!reason.trim()?'not-allowed':'pointer',fontSize:'13px',fontWeight:700,fontFamily:sans}}>{busy?'Suspending…':'Suspend'}</button>
        </div>
      </div>
    </div>
  )
  ...
  return(
    <div onClick={onClose} style={{position:'fixed',inset:0,background:'rgba(0,0,0,.60)',zIndex:320,display:'flex',alignItems:'center',justifyContent:'center',padding:'24px'}}>
      <div onClick={e=>e.stopPropagation()} style={{background:'#fff',borderRadius:'10px',padding:'28px',width:'100%',maxWidth:'480px',boxShadow:'0 20px 60px rgba(0,0,0,.22)',fontFamily:sans}}>
        ...
        <p style={{fontSize:'13px',color:'var(--portal-ink-mid)',margin:'0 0 16px',lineHeight:1.6}}>This will move the service to <strong>Denied</strong> status. The provider will be notified and may submit an appeal.</p>
        <label style={{display:'block',marginBottom:'12px'}}>
          <div style={{fontSize:'11px',fontWeight:700,color:'var(--portal-ink-mid)',textTransform:'uppercase',letterSpacing:'.05em',marginBottom:'5px'}}>Denial Category <span style={{color:RED}}>*</span></div>
          <select value={category} onChange={e=>setCategory(e.target.value)} style={{width:'100%',padding:'9px 12px',borderRadius:'7px',border:'1px solid var(--portal-rule)',fontSize:'13px',fontFamily:sans,background:'#fff'}}>
            {DENIAL_CATEGORIES.map(c=><option key={c.value} value={c.value}>{c.label}</option>)}
          </select>
        </label>
        <label style={{display:'block',marginBottom:'16px'}}>
          <div style={{fontSize:'11px',fontWeight:700,color:'var(--portal-ink-mid)',textTransform:'uppercase',letterSpacing:'.05em',marginBottom:'5px'}}>Reason <span style={{color:RED}}>*</span></div>
          <textarea rows={3} value={reason} onChange={e=>setReason(e.target.value)} placeholder="Specific, actionable feedback for the provider…" style={{width:'100%',boxSizing:'border-box',border:'1px solid var(--portal-rule)',borderRadius:'7px',padding:'10px 12px',fontSize:'13px',fontFamily:sans,resize:'vertical',outline:'none',lineHeight:1.55}}/>
        </label>
        <div style={{display:'flex',gap:'8px',justifyContent:'flex-end'}}>
          <button onClick={onClose} style={{padding:'8px 18px',borderRadius:'6px',border:'1px solid var(--portal-rule)',background:'#fff',color:'var(--portal-ink-mid)',cursor:'pointer',fontSize:'13px',fontWeight:600,fontFamily:sans}}>Cancel</button>
          <button disabled={!reason.trim()||busy} onClick={async()=>{setBusy(true);await onConfirm(gig.id,reason.trim(),category);setBusy(false)}} style={{padding:'8px 18px',borderRadius:'6px',border:'none',background:!reason.trim()?'var(--portal-ink-soft)':RED,color:'#fff',cursor:!reason.trim()?'not-allowed':'pointer',fontSize:'13px',fontWeight:700,fontFamily:sans}}>{busy?'Denying…':'Deny Service'}</button>
        </div>
      </div>
    </div>
  )
    ...
    <div onClick={onClose} style={{position:'fixed',inset:0,background:'rgba(0,0,0,.60)',zIndex:320,display:'flex',alignItems:'center',justifyContent:'center',padding:'24px'}}>
      <div onClick={e=>e.stopPropagation()} style={{background:'#fff',borderRadius:'10px',padding:'28px',width:'100%',maxWidth:'420px',boxShadow:'0 20px 60px rgba(0,0,0,.22)',fontFamily:sans}}>
        ...
        <div style={{display:'flex',gap:'8px',marginBottom:'14px'}}>
          {[{v:'boost',l:'🚀 Boost'},{v:'featured',l:'⭐ Feature'}].map(o=><button key={o.v} onClick={()=>setType(o.v)} style={{flex:1,padding:'8px',borderRadius:'6px',border:`1px solid ${type===o.v?NAVY:'var(--portal-rule)'}`,background:type===o.v?NAVY:'#fff',color:type===o.v?'#fff':'var(--portal-ink-mid)',cursor:'pointer',fontSize:'13px',fontWeight:600,fontFamily:sans}}>{o.l}</button>)}
        </div>
        <label style={{display:'block',marginBottom:'16px'}}>
          <div style={{fontSize:'11px',fontWeight:700,color:'var(--portal-ink-mid)',textTransform:'uppercase',letterSpacing:'.05em',marginBottom:'5px'}}>Duration</div>
          <select value={days} onChange={e=>setDays(Number(e.target.value))} style={{width:'100%',padding:'9px 12px',borderRadius:'7px',border:'1px solid var(--portal-rule)',fontSize:'13px',fontFamily:sans,background:'#fff'}}>
  ...
  return(
    <div onClick={onClose} style={{position:'fixed',inset:0,background:'rgba(0,0,0,.60)',zIndex:320,display:'flex',alignItems:'center',justifyContent:'center',padding:'24px'}}>
      <div onClick={e=>e.stopPropagation()} style={{background:'#fff',borderRadius:'10px',padding:'28px',width:'100%',maxWidth:'420px',boxShadow:'0 20px 60px rgba(0,0,0,.22)',fontFamily:sans}}>
        ...
        <p style={{fontSize:'13px',color:'var(--portal-ink-mid)',margin:'0 0 16px'}}>Current: <strong>{Number(gig.rank_score||0).toFixed(4)}</strong>. Enter 0–1 (higher = better ranking).</p>
        <label style={{display:'block',marginBottom:'12px'}}>
          <div style={{fontSize:'11px',fontWeight:700,color:'var(--portal-ink-mid)',textTransform:'uppercase',letterSpacing:'.05em',marginBottom:'5px'}}>New Score (0–1)</div>
          <input type="number" min="0" max="1" step="0.0001" value={score} onChange={e=>setScore(e.target.value)} style={{width:'100%',boxSizing:'border-box',padding:'9px 12px',borderRadius:'7px',border:'1px solid var(--portal-rule)',fontSize:'13px',fontFamily:sans,outline:'none'}}/>
          {parsed>0&&<div style={{display:'flex',height:'6px',background:'var(--portal-rule-soft)',borderRadius:'3px',marginTop:'8px',overflow:'hidden'}}><div style={{width:`${parsed*100}%`,background:parsed>=.7?GREEN:parsed>=.4?AMBER:RED,transition:'width .2s'}}/></div>}
        </label>
        <label style={{display:'block',marginBottom:'16px'}}>
          <div style={{fontSize:'11px',fontWeight:700,color:'var(--portal-ink-mid)',textTransform:'uppercase',letterSpacing:'.05em',marginBottom:'5px'}}>Reason (audit)</div>
          <input value={reason} onChange={e=>setReason(e.target.value)} placeholder="e.g. Penalty for duplicate content" style={{width:'100%',boxSizing:'border-box',padding:'9px 12px',borderRadius:'7px',border:'1px solid var(--portal-rule)',fontSize:'13px',fontFamily:sans,outline:'none'}}/>
        </label>
        <div style={{display:'flex',gap:'8px',justifyContent:'flex-end'}}>
          <button onClick={onClose} style={{padding:'8px 18px',borderRadius:'6px',border:'1px solid var(--portal-rule)',background:'#fff',color:'var(--portal-ink-mid)',cursor:'pointer',fontSize:'13px',fontWeight:600,fontFamily:sans}}>Cancel</button>
        ...
        <div style={{flex:1,overflowY:'auto',padding:'20px 24px'}}>
          ...
          {section==='overview'&&(
            <div style={{display:'flex',flexDirection:'column',gap:'14px'}}>
              {/* Provider */}
              <div style={{background:'var(--portal-bg)',border:'1px solid var(--portal-rule)',borderRadius:'8px',padding:'14px 16px'}}>
                <div style={{fontSize:'10px',fontWeight:700,color:'var(--portal-ink-soft)',textTransform:'uppercase',letterSpacing:'.06em',marginBottom:'8px'}}>Provider</div>
                <div style={{display:'flex',alignItems:'center',gap:'10px'}}>
                  ...
                  <div style={{flex:1,minWidth:0}}>
                    ...
                    <div style={{fontSize:'11px',color:'var(--portal-ink-soft)'}}>{gig.provider?.email}</div>
                  </div>
                  {gig.seller_level&&<LevelBadge level={gig.seller_level}/>}
                </div>
              </div>
              ...
              <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'8px'}}>
                {[
                  ...
                  <div key={l} style={{background:'var(--portal-bg)',border:'1px solid var(--portal-rule)',borderRadius:'7px',padding:'10px 12px'}}>
                    <div style={{fontSize:'10px',fontWeight:700,color:'var(--portal-ink-soft)',textTransform:'uppercase',letterSpacing:'.06em'}}>{l}</div>
              ...
              {gig.auto_flag_score>0&&(
                <div style={{background:'#FEF5E4',border:'1px solid rgba(139,94,10,.22)',borderRadius:'8px',padding:'14px 16px'}}>
                  ...
                  {(gig.auto_flag_reasons||[]).map((f,i)=>(
                    <div key={i} style={{fontSize:'12px',color:'var(--portal-ink-mid)',padding:'3px 0',borderBottom:'1px solid rgba(139,94,10,.10)'}}>{f.type}: {f.description}</div>
                  ))}
                </div>
              )}
              {/* Tags */}
              {gig.tags?.length>0&&(
                <div>
                  <div style={{fontSize:'10px',fontWeight:700,color:'var(--portal-ink-soft)',textTransform:'uppercase',letterSpacing:'.06em',marginBottom:'6px'}}>Tags</div>
                  <div style={{display:'flex',gap:'5px',flexWrap:'wrap'}}>{gig.tags.map(t=><span key={t} style={{padding:'2px 7px',borderRadius:'4px',background:'var(--portal-rule-soft)',border:'1px solid var(--portal-rule)',fontSize:'12px',color:'var(--portal-ink-mid)'}}>{t}</span>)}</div>
                </div>
              )}
              {/* Gallery */}
              {gig.gallery_images?.length>0&&(
                <div>
                  <div style={{fontSize:'10px',fontWeight:700,color:'var(--portal-ink-soft)',textTransform:'uppercase',letterSpacing:'.06em',marginBottom:'6px'}}>Gallery ({gig.gallery_images.length})</div>
                  <div style={{display:'flex',gap:'6px',flexWrap:'wrap'}}>{gig.gallery_images.slice(0,4).map((img,i)=>{const url=typeof img==='string'?img:img?.url;return url?<img key={i} src={url} alt={gig.title || 'Gallery image'} style={{width:'70px',height:'70px',objectFit:'cover',borderRadius:'6px',border:'1px solid var(--portal-rule)'}}/>:null})}</div>
                </div>
              )}
            </div>
          )}
          ...
          {section==='content'&&(
            <div style={{display:'flex',flexDirection:'column',gap:'14px'}}>
              {gig.pitch&&<div><div style={{fontSize:'10px',fontWeight:700,color:'var(--portal-ink-soft)',textTransform:'uppercase',letterSpacing:'.06em',marginBottom:'5px'}}>Pitch ({gig.pitch.length} chars)</div><p style={{margin:0,fontSize:'13px',color:NAVY,lineHeight:1.6,fontStyle:'italic',background:'var(--portal-bg)',border:'1px solid var(--portal-rule)',borderRadius:'7px',padding:'12px 14px'}}>{gig.pitch}</p></div>}
              {gig.description&&<div><div style={{fontSize:'10px',fontWeight:700,color:'var(--portal-ink-soft)',textTransform:'uppercase',letterSpacing:'.06em',marginBottom:'5px'}}>Description ({gig.description.length} chars)</div><p style={{margin:0,fontSize:'13px',color:'var(--portal-ink-mid)',lineHeight:1.7,maxHeight:'220px',overflowY:'auto',background:'var(--portal-bg)',border:'1px solid var(--portal-rule)',borderRadius:'7px',padding:'12px 14px',whiteSpace:'pre-wrap'}}>{gig.description}</p></div>}
              {gig.requirements&&<div><div style={{fontSize:'10px',fontWeight:700,color:'var(--portal-ink-soft)',textTransform:'uppercase',letterSpacing:'.06em',marginBottom:'5px'}}>Requirements</div><p style={{margin:0,fontSize:'13px',color:'var(--portal-ink-mid)',lineHeight:1.6,background:'var(--portal-bg)',border:'1px solid var(--portal-rule)',borderRadius:'7px',padding:'12px 14px',whiteSpace:'pre-wrap'}}>{gig.requirements}</p></div>}
              {gig.faq?.length>0&&<div><div style={{fontSize:'10px',fontWeight:700,color:'var(--portal-ink-soft)',textTransform:'uppercase',letterSpacing:'.06em',marginBottom:'6px'}}>FAQ ({gig.faq.length})</div>{gig.faq.map((f,i)=><div key={i} style={{marginBottom:'8px',background:'var(--portal-bg)',border:'1px solid var(--portal-rule)',borderRadius:'7px',padding:'10px 14px'}}><div style={{fontWeight:600,fontSize:'13px',color:NAVY,marginBottom:'2px'}}>{f.q||f.question}</div><div style={{fontSize:'12px',color:'var(--portal-ink-mid)',lineHeight:1.55}}>{f.a||f.answer}</div></div>)}</div>}
              {gig.seo_title&&<div style={{background:'var(--portal-bg)',border:'1px solid var(--portal-rule)',borderRadius:'7px',padding:'12px 14px'}}><div style={{fontSize:'10px',fontWeight:700,color:'var(--portal-ink-soft)',textTransform:'uppercase',letterSpacing:'.06em',marginBottom:'5px'}}>SEO</div><div style={{fontSize:'13px',fontWeight:600,color:NAVY,marginBottom:'2px'}}>{gig.seo_title}</div><div style={{fontSize:'12px',color:'var(--portal-ink-mid)'}}>{gig.seo_description}</div></div>}
            </div>
          )}
          ...
          {section==='pricing'&&(
            <div style={{display:'flex',flexDirection:'column',gap:'10px'}}>
              {tiers.length===0?<p style={{color:'var(--portal-ink-soft)',fontSize:'13px'}}>No tiers.</p>:tiers.map(t=>(
                <div key={t.id||t.tier} style={{background:'var(--portal-bg)',border:`1px solid ${t.is_active?'var(--portal-rule)':'rgba(0,0,0,.06)'}`,borderRadius:'8px',padding:'14px 16px',opacity:t.is_active?1:.55}}>
                  <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',marginBottom:'6px'}}>
                    <div><span style={{fontSize:'10px',fontWeight:700,letterSpacing:'.08em',textTransform:'uppercase',color:'var(--portal-ink-soft)'}}>{t.tier}</span><div style={{fontWeight:700,fontSize:'15px',color:NAVY}}>{t.title}</div></div>
                    <div style={{textAlign:'right'}}><div style={{fontWeight:800,fontSize:'18px',color:NAVY}}>{fmt(t.price)}</div>{!t.is_active&&<span style={{fontSize:'11px',color:'var(--portal-ink-soft)'}}>Inactive</span>}</div>
                  </div>
                  <div style={{display:'flex',gap:'12px',fontSize:'12px',color:'var(--portal-ink-mid)'}}>
                    <span>🕐 {t.delivery_days}d</span>
                    <span>🔄 {t.revisions===0?'Unlimited':`${t.revisions} rev`}</span>
                  </div>
                </div>
              ))}
              <div style={{borderTop:'1px solid var(--portal-rule)',paddingTop:'10px',display:'flex',justifyContent:'space-between',fontSize:'13px',color:'var(--portal-ink-mid)'}}>
            ...
            <div style={{display:'flex',flexDirection:'column',gap:'10px'}}>
              <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'10px'}}>
                {[
                  ...
                  <div key={m.l} style={{background:'var(--portal-bg)',border:'1px solid var(--portal-rule)',borderRadius:'8px',padding:'14px 16px'}}>
                    <div style={{fontSize:'10px',fontWeight:700,color:'var(--portal-ink-soft)',textTransform:'uppercase',letterSpacing:'.06em',marginBottom:'5px'}}>{m.i} {m.l}</div>
              ...
              {gig.content_score!=null&&(
                <div style={{background:'var(--portal-bg)',border:'1px solid var(--portal-rule)',borderRadius:'8px',padding:'14px 16px'}}>
                  <div style={{display:'flex',justifyContent:'space-between',marginBottom:'6px'}}><span style={{fontSize:'12px',fontWeight:600,color:'var(--portal-ink-mid)'}}>Profile Completeness</span><span style={{fontSize:'13px',fontWeight:700,color:NAVY}}>{gig.content_score}%</span></div>
            ...
            <div style={{display:'flex',flexDirection:'column',gap:'12px'}}>
              ...
              {gig.status==='suspended'&&(
                <div style={{background:'#FEF5E4',border:'1px solid rgba(139,94,10,.25)',borderRadius:'8px',padding:'14px 16px'}}>
                  ...
                  <div style={{fontSize:'13px',color:'var(--portal-ink-mid)',lineHeight:1.55,marginBottom:'5px'}}>{gig.gig_status_reason||'No reason recorded.'}</div>
                  {gig.suspended_at&&<div style={{fontSize:'11px',color:'var(--portal-ink-soft)'}}>{ago(gig.suspended_at)}{gig.suspended_by_name?` by ${gig.suspended_by_name}`:''}</div>}
                </div>
              )}
              {gig.status==='denied'&&(
                <div style={{background:'#FAEAEA',border:'1px solid rgba(139,26,26,.22)',borderRadius:'8px',padding:'14px 16px'}}>
                  ...
                  <div style={{fontSize:'13px',color:'var(--portal-ink-mid)',lineHeight:1.55}}>{gig.denial_reason||gig.gig_status_reason||'No reason recorded.'}</div>
                </div>
              )}
              {gig.status==='appeal_pending'&&gig.appeal_reason&&(
                <div style={{background:'#F5F3FF',border:'1px solid rgba(61,43,107,.22)',borderRadius:'8px',padding:'14px 16px'}}>
                  ...
                  <div style={{fontSize:'13px',color:'var(--portal-ink-mid)',lineHeight:1.55}}>{gig.appeal_reason}</div>
                </div>
              )}
              {/* Action buttons */}
              <div style={{background:'var(--portal-bg)',border:'1px solid var(--portal-rule)',borderRadius:'8px',padding:'14px 16px'}}>
                <div style={{fontSize:'10px',fontWeight:700,color:'var(--portal-ink-soft)',textTransform:'uppercase',letterSpacing:'.06em',marginBottom:'10px'}}>Moderation Actions</div>
          ...
          {section==='audit'&&(
            <div style={{display:'flex',flexDirection:'column',gap:'8px'}}>
              <div style={{fontSize:'12px',color:'var(--portal-ink-soft)',marginBottom:'4px'}}>Last 5 admin actions on this service.</div>
              {!gig.audit_log?.length?<p style={{color:'var(--portal-ink-soft)',fontSize:'13px',padding:'16px',textAlign:'center',background:'var(--portal-bg)',borderRadius:'8px'}}>No moderation history.</p>:gig.audit_log.map((e,i)=>(
                <div key={i} style={{background:'var(--portal-bg)',border:'1px solid var(--portal-rule)',borderRadius:'7px',padding:'12px 14px'}}>
                  <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',gap:'8px',marginBottom:'3px'}}>
                    ...
                    <span style={{fontSize:'11px',color:'var(--portal-ink-soft)',flexShrink:0}}>{ago(e.created_at)}</span>
                  </div>
                  {e.reason&&<div style={{fontSize:'12px',color:'var(--portal-ink-mid)',lineHeight:1.5,fontStyle:'italic'}}>"{e.reason}"</div>}
                </div>
              ))}
            </div>
          )}
          ...
          {section==='history'&&(
            <div style={{display:'flex',flexDirection:'column',gap:'8px'}}>
              <div style={{fontSize:'12px',color:'var(--portal-ink-soft)',marginBottom:'4px'}}>Version history — every edit to this service.</div>
              {history===null?<p style={{color:'var(--portal-ink-soft)',fontSize:'13px',padding:'16px',textAlign:'center'}}>Loading…</p>:history.length===0?<p style={{color:'var(--portal-ink-soft)',fontSize:'13px',padding:'16px',textAlign:'center',background:'var(--portal-bg)',borderRadius:'8px'}}>No version history yet.</p>:history.map((v,i)=>(
                <div key={i} style={{background:'var(--portal-bg)',border:'1px solid var(--portal-rule)',borderRadius:'7px',padding:'12px 14px'}}>
                  <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',gap:'8px',marginBottom:'3px'}}>
                    <span style={{fontWeight:700,fontSize:'13px',color:NAVY}}>v{v.version_num} — {v.change_summary||'Updated'}</span>
                    <span style={{fontSize:'11px',color:'var(--portal-ink-soft)',flexShrink:0}}>{ago(v.created_at)}</span>
                  </div>
                  <div style={{fontSize:'11px',color:'var(--portal-ink-soft)'}}>{v.changed_by_role==='admin'?'Admin':v.changed_by_role||'Provider'} edit</div>
                </div>
              ))}
            </div>
          )}
...
function ModQueueTab({onSelectGig,onAction}) {
  ...
  if(loading) return <div style={{padding:'40px',textAlign:'center',color:'var(--portal-ink-soft)',fontSize:'14px'}}>Loading moderation queue…</div>
  ...
  const GigQueueRow=({gig,badge,badgeColor,actions})=>(
    <div style={{display:'flex',alignItems:'center',gap:'14px',padding:'13px 16px',borderBottom:'1px solid #F2EFE9',background:'#fff'}}>
      <div style={{flex:1,minWidth:0}}>
        ...
        <div style={{fontSize:'11px',color:'var(--portal-ink-soft)'}}>{gig.provider?.name||'—'} · {gig.category||'—'} · {ago(gig.created_at)}</div>
        <ScoreBar score={gig.content_score} compact/>
      </div>
      <div style={{display:'flex',gap:'5px',flexShrink:0}}>
        <button onClick={()=>onSelectGig(gig)} style={{padding:'5px 11px',borderRadius:'5px',border:'1px solid var(--portal-rule)',background:'var(--portal-bg)',color:NAVY,cursor:'pointer',fontSize:'12px',fontWeight:600,fontFamily:sans}}>Review</button>
        {actions}
      </div>
    </div>
  )
    ...
    <div style={{display:'flex',flexDirection:'column',gap:'20px'}}>
      ...
      <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',gap:'12px'}}>
        <div>
          ...
          <p style={{color:'var(--portal-ink-soft)',fontSize:'13px',margin:'3px 0 0'}}>{(counts.pending_review||0)+(counts.appeals||0)} requiring decision · {counts.auto_flagged||0} auto-flagged</p>
        </div>
        <div style={{display:'flex',gap:'8px',alignItems:'center'}}>
          ...
          <button onClick={load} style={{padding:'8px 14px',borderRadius:'6px',border:'1px solid var(--portal-rule)',background:'#fff',color:NAVY,cursor:'pointer',fontSize:'13px',fontWeight:600,fontFamily:sans}}>↻</button>
        </div>
      </div>

      {/* Priority order: Pending Review → Appeals → Auto-flagged */}
      {pendingReview.length>0&&(
        <div style={{background:'#fff',border:'1px solid var(--portal-rule)',borderRadius:'8px',overflow:'hidden',boxShadow:'0 1px 4px rgba(27,45,79,.05)'}}>
      ...
      {pendingReview.length===0&&appeals.length===0&&autoFlagged.length===0&&(
        <div style={{background:'#fff',border:'1px dashed #C8C2B6',borderRadius:'8px',padding:'48px 24px',textAlign:'center'}}>
          ...
          <div style={{fontSize:'13px',color:'var(--portal-ink-soft)',lineHeight:1.6}}>No pending reviews, appeals, or auto-flagged gigs. Run auto-flag to scan active listings.</div>
        </div>
      )}
    </div>
  )
...
function StatsTab() {
  ...
  if(loading) return <div style={{padding:'40px',textAlign:'center',color:'var(--portal-ink-soft)'}}>Loading stats…</div>

  const Kpi=({label,value,accent='#0F172A',icon})=><div style={{background:'#fff',border:`1px solid var(--portal-rule)`,borderTop:`3px solid ${accent}`,borderRadius:'8px',padding:'16px 18px'}}>
    <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:'8px'}}><span style={{fontSize:'11px',fontWeight:700,textTransform:'uppercase',letterSpacing:'.08em',color:'var(--portal-ink-soft)'}}>{label}</span><span style={{fontSize:'15px',opacity:.4}}>{icon}</span></div>
  ...
  return(
    <div style={{display:'flex',flexDirection:'column',gap:'20px'}}>
      ...
      {/* Status breakdown */}
      <div style={{background:'#fff',border:'1px solid var(--portal-rule)',borderRadius:'8px',padding:'20px'}}>
          ...
          {Object.entries(stats?.by_status||{}).map(([status,count])=>{
            ...
            return(
              <div key={status} style={{display:'flex',alignItems:'center',gap:'10px',padding:'6px 0',borderBottom:'1px solid #F2EFE9'}}>
                ...
                <span style={{fontSize:'13px',color:'var(--portal-ink-mid)',flex:1,textTransform:'capitalize'}}>{status.replace(/_/g,' ')}</span>
                <div style={{width:'80px',height:'5px',background:'var(--portal-rule-soft)',borderRadius:'2px',overflow:'hidden'}}><div style={{height:'100%',width:`${(count/total)*100}%`,background:cfg.dot,borderRadius:'2px'}}/></div>
      ...
      {(stats?.by_category||[]).length>0&&(
        <div style={{background:'#fff',border:'1px solid var(--portal-rule)',borderRadius:'8px',overflow:'hidden'}}>
            ...
            <tbody>
              {stats.by_category.slice(0,10).map((c,i)=>(
                <tr key={c.category} style={{background:i%2===0?'#fff':'#FAFAF8',borderBottom:'1px solid #F2EFE9'}}>
                  ...
                  <td style={{padding:'9px 14px',fontSize:'13px',color:'var(--portal-ink-soft)',textAlign:'right'}}>{c.count} gigs</td>
  ...
  return(
    <div style={{display:'flex',flexDirection:'column',gap:'20px'}}>
      ...
      <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(240px,1fr))',gap:'14px'}}>
        {[
          {level:'new_seller',label:'New Seller',color:'var(--portal-ink-soft)',icon:'🌱',min_orders:0,min_rating:'—',completion:'—',tenure:'< 60 days',multiplier:'0.80×'},
          ...
          <div key={l.level} style={{background:'#fff',border:'1px solid var(--portal-rule)',borderTop:`3px solid ${l.color}`,borderRadius:'8px',padding:'18px'}}>
            <div style={{display:'flex',alignItems:'center',gap:'8px',marginBottom:'12px'}}>
              <span style={{fontSize:'20px'}}>{l.icon}</span>
              <div>
                <div style={{fontWeight:700,fontSize:'15px',color:NAVY}}>{l.label}</div>
                <div style={{fontSize:'11px',color:'var(--portal-ink-soft)'}}>Rank multiplier: <strong style={{color:l.color}}>{l.multiplier}</strong></div>
              </div>
            </div>
            {[['Min Orders',l.min_orders],['Min Rating',l.min_rating],['Completion',l.completion],['Tenure',l.tenure]].map(([k,v])=>(
              <div key={k} style={{display:'flex',justifyContent:'space-between',padding:'5px 0',borderBottom:'1px solid #F2EFE9',fontSize:'12px'}}>
                <span style={{color:'var(--portal-ink-soft)'}}>{k}</span>
      ...
      <div style={{background:'var(--portal-bg)',border:'1px solid var(--portal-rule)',borderLeft:`4px solid ${GOLD}`,borderRadius:'8px',padding:'16px 18px'}}>
        ...
        <p style={{fontSize:'13px',color:'var(--portal-ink-mid)',lineHeight:1.65,margin:0}}>
...
export default function AdminGigsManager({ formatPrimary }) {
  ...
  return(
    <div style={{padding:'28px',display:'flex',flexDirection:'column',gap:'22px',fontFamily:sans,background:'var(--portal-bg)',minHeight:'100vh'}}>

      {/* Header */}
      <div style={{display:'flex',alignItems:'flex-end',justifyContent:'space-between',gap:'12px',flexWrap:'wrap'}}>
        <div>
          <div style={{fontSize:'11px',fontWeight:700,textTransform:'uppercase',letterSpacing:'.14em',color:'var(--portal-ink-soft)',marginBottom:'4px'}}>Marketplace</div>
          ...
          <p style={{color:'var(--portal-ink-soft)',fontSize:'13px',margin:'5px 0 0'}}>Fiverr-grade gig management — moderation queue, ranking, content scoring, version history.</p>
        </div>
        <button onClick={load} style={{padding:'8px 16px',borderRadius:'6px',border:'1px solid var(--portal-rule)',background:'#fff',color:NAVY,cursor:'pointer',fontSize:'13px',fontWeight:600,fontFamily:sans}}>↻ Refresh</button>
      </div>
      ...
      {/* Tabs */}
      <div style={{display:'flex',borderBottom:'1px solid var(--portal-rule)',gap:0,overflowX:'auto',background:'#fff',borderRadius:'8px 8px 0 0',boxShadow:'0 1px 3px rgba(27,45,79,.05)'}}>
        {TABS.map(t=>(
          <button key={t.id} onClick={()=>setTab(t.id)} style={{display:'inline-flex',alignItems:'center',gap:'5px',padding:'12px 18px',fontSize:'13px',fontWeight:tab===t.id?600:400,color:tab===t.id?NAVY:'var(--portal-ink-soft)',background:'none',border:'none',borderBottom:tab===t.id?`2px solid ${NAVY}`:'2px solid transparent',cursor:'pointer',whiteSpace:'nowrap',fontFamily:sans}}>
        ...
        <>
          ...
          <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(120px,1fr))',gap:'10px'}}>
            {[{k:'all',l:'Total',c:NAVY,n:Object.values(byStatus).reduce((s,v)=>s+v,0)},{k:'active',l:'Active',c:GREEN,n:byStatus.active||0},{k:'pending_review',l:'Pending',c:'#3B82F6',n:byStatus.pending_review||0},{k:'suspended',l:'Suspended',c:AMBER,n:byStatus.suspended||0},{k:'denied',l:'Denied',c:RED,n:byStatus.denied||0},{k:'archived',l:'Archived',c:PURPLE,n:byStatus.archived||0}].map(s=>(
              <button key={s.k} onClick={()=>{setStatusFilter(s.k);setLocalPage(1)}} style={{padding:'12px 14px',borderRadius:'8px',border:`1px solid ${statusFilter===s.k?s.c:'var(--portal-rule)'}`,background:statusFilter===s.k?`${s.c}12`:'#fff',cursor:'pointer',textAlign:'left',fontFamily:sans}}>
                <div style={{fontSize:'11px',fontWeight:700,textTransform:'uppercase',letterSpacing:'.06em',color:'var(--portal-ink-soft)',marginBottom:'4px'}}>{s.l}</div>
          ...
          <div style={{display:'flex',gap:'10px',flexWrap:'wrap',alignItems:'center'}}>
            <input value={searchQ} onChange={e=>{setSearchQ(e.target.value);setLocalPage(1)}} placeholder="Search title, provider, category…" style={{flex:'1 1 220px',maxWidth:'340px',padding:'8px 12px',borderRadius:'7px',border:'1px solid var(--portal-rule)',fontSize:'13px',fontFamily:sans,outline:'none'}}/>
            <select value={typeFilter} onChange={e=>{setTypeFilter(e.target.value);setLocalPage(1)}} style={{padding:'8px 12px',borderRadius:'7px',border:'1px solid var(--portal-rule)',fontSize:'13px',fontFamily:sans,background:'#fff',cursor:'pointer'}}>
              <option value="all">All types</option><option value="attorney">Attorneys</option><option value="consultant">Consultants</option>
            </select>
            <select value={categoryFilter} onChange={e=>{setCategoryFilter(e.target.value);setLocalPage(1)}} style={{padding:'8px 12px',borderRadius:'7px',border:'1px solid var(--portal-rule)',fontSize:'13px',fontFamily:sans,background:'#fff',cursor:'pointer'}}>
              <option value="all">All categories</option>{categories.map(c=><option key={c} value={c}>{c}</option>)}
            </select>
            <label style={{display:'flex',alignItems:'center',gap:'6px',fontSize:'13px',color:'var(--portal-ink-mid)',cursor:'pointer'}}>
            ...
            <span style={{marginLeft:'auto',fontSize:'12px',color:'var(--portal-ink-soft)'}}>{serverTotal.toLocaleString()} results{selectedRows.size>0&&<span style={{marginLeft:'8px',fontWeight:700,color:NAVY}}>· {selectedRows.size} selected</span>}</span>
          </div>
          ...
          {error?<div style={{background:'#FAEAEA',border:'1px solid rgba(139,26,26,.20)',borderRadius:'8px',padding:'20px',fontSize:'14px',color:RED}}>{error} — <button onClick={load} style={{background:'none',border:'none',color:RED,cursor:'pointer',textDecoration:'underline',fontSize:'13px'}}>Retry</button></div>:(
            <div style={{background:'#fff',border:'1px solid var(--portal-rule)',borderRadius:'8px',overflow:'hidden',boxShadow:'0 1px 4px rgba(27,45,79,.06)'}}>
              <div style={{overflowX:'auto'}}>
                <table style={{width:'100%',borderCollapse:'collapse',minWidth:'960px'}}>
                  ...
                  <tbody>
                    {loading?[1,2,3,4,5].map(i=><tr key={i} style={{background:i%2===0?'#fff':'#FAFAF8'}}>{Array.from({length:12}).map((_,j)=><td key={j} style={{padding:'13px'}}><div style={{height:'13px',background:'var(--portal-rule-soft)',borderRadius:'3px',width:j===0?'80%':'55%'}}/></td>)}</tr>)
                    :pagedGigs.length===0?<tr><td colSpan={12} style={{padding:'48px',textAlign:'center',color:'var(--portal-ink-soft)',fontSize:'14px'}}>No services match the current filters.</td></tr>
                    :pagedGigs.map((g,i)=>{
                      ...
                      return(
                        <tr key={g.id} style={{background:isSelected?'rgba(196,164,90,.07)':i%2===0?'#fff':'#FAFAF8',borderBottom:'1px solid #F2EFE9',transition:'background 80ms'}}>
                          ...
                          <td style={{padding:'11px 12px'}}>
                            ...
                            <div style={{fontSize:'11px',color:'var(--portal-ink-soft)',overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap',maxWidth:'200px'}}>{g.provider?.name||'—'}</div>
                          </td>
                          ...
                          <td style={{padding:'11px 12px',fontSize:'12px',color:'var(--portal-ink-soft)'}}>{g.category||'—'}</td>
                          ...
                          <td style={{padding:'11px 12px',fontSize:'12px',color:'var(--portal-ink-soft)',fontVariantNumeric:'tabular-nums'}}>{g.rank_score!=null?Number(g.rank_score).toFixed(3):'—'}</td>
                          <td style={{padding:'11px 12px'}}><FlagBadge score={g.auto_flag_score}/></td>
                          <td style={{padding:'11px 12px',fontSize:'13px',color:'var(--portal-ink-soft)',textAlign:'right',fontVariantNumeric:'tabular-nums'}}>{fmtN(g.impressions,true)}</td>
                          <td style={{padding:'11px 12px',fontSize:'12px',color:'var(--portal-ink-soft)',whiteSpace:'nowrap'}}>{ago(g.created_at)}</td>
                          <td style={{padding:'11px 12px',whiteSpace:'nowrap'}}>
                            <div style={{display:'flex',gap:'5px'}}>
                              <button onClick={()=>setSelectedGig(g)} style={{padding:'5px 10px',borderRadius:'5px',border:'1px solid var(--portal-rule)',background:'var(--portal-bg)',color:NAVY,cursor:'pointer',fontSize:'12px',fontWeight:600,fontFamily:sans}}>View</button>
                  ...
                  {pagedGigs.length>0&&!loading&&(
                    <tfoot><tr style={{background:'#F8F7F4',borderTop:'2px solid #DDD8CE'}}>
                      <td colSpan={5} style={{padding:'9px 12px',fontSize:'12px',color:'var(--portal-ink-soft)',fontWeight:600}}>{(localPage-1)*PER_PAGE+1}–{Math.min(localPage*PER_PAGE,serverTotal)} of {serverTotal.toLocaleString()}</td>
                      <td colSpan={7}/>
                    </tr></tfoot>
                  )}
                </table>
              ...
              {totalPages>1&&(
                <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',padding:'12px 16px',borderTop:'1px solid #F2EFE9',background:'#FAFAF8'}}>
                  <button onClick={()=>setLocalPage(p=>Math.max(1,p-1))} disabled={localPage===1} style={{padding:'6px 14px',borderRadius:'6px',border:'1px solid var(--portal-rule)',background:localPage===1?'var(--portal-bg)':'#fff',color:localPage===1?'var(--portal-ink-soft)':NAVY,cursor:localPage===1?'not-allowed':'pointer',fontSize:'13px',fontWeight:600,fontFamily:sans}}>← Prev</button>
                  <div style={{display:'flex',gap:'4px'}}>
                    {Array.from({length:totalPages},(_, i)=>i+1).filter(p=>p===1||p===totalPages||Math.abs(p-localPage)<=1).reduce((acc,p,idx,arr)=>{if(idx>0&&p-arr[idx-1]>1)acc.push('…');acc.push(p);return acc},[]).map((p,i)=>p==='…'?<span key={`e${i}`} style={{padding:'6px 4px',color:'var(--portal-ink-soft)',fontSize:'13px'}}>…</span>:<button key={p} onClick={()=>setLocalPage(p)} style={{width:'32px',height:'32px',borderRadius:'6px',border:`1px solid ${p===localPage?NAVY:'var(--portal-rule)'}`,background:p===localPage?NAVY:'#fff',color:p===localPage?'#fff':NAVY,cursor:'pointer',fontSize:'13px',fontWeight:p===localPage?700:400,fontFamily:sans}}>{p}</button>)}
                  </div>
                  <button onClick={()=>setLocalPage(p=>Math.min(totalPages,p+1))} disabled={localPage===totalPages} style={{padding:'6px 14px',borderRadius:'6px',border:'1px solid var(--portal-rule)',background:localPage===totalPages?'var(--portal-bg)':'#fff',color:localPage===totalPages?'var(--portal-ink-soft)':NAVY,cursor:localPage===totalPages?'not-allowed':'pointer',fontSize:'13px',fontWeight:600,fontFamily:sans}}>Next →</button>
                </div>
              )}
            </div>
          )}
