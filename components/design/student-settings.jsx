    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
      <Section title="Sign-in security" sub="Your authentication is managed by Clerk.">
        <div style={{ display: 'grid', gap: 10 }}>
          <div style={infoRow}><span style={infoKey}>Email</span><span style={infoVal}>{data.profile.email}</span></div>
          <div style={infoRow}><span style={infoKey}>Account ID</span><span style={{ ...infoVal, fontFamily: MONO, fontSize: 11 }}>{data.profile.id}</span></div>
          ...
          {data.profile.vertical && <div style={infoRow}><span style={infoKey}>Vertical</span><span style={infoVal}><Badge color="purple" style={{ fontSize: 10 }}>{data.profile.vertical}</Badge></span></div>}
        </div>
      </Section>
      ...
      <Section title="Security actions">
        <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
          ...
          <Btn variant="ghost" size="sm" onClick={() => flash('ok', 'Active sessions list is available in your Clerk dashboard.')}>Active sessions</Btn>
        </div>
        <div style={{ marginTop: 14, padding: 12, background: `${AMBER}08`, border: `1px solid ${AMBER}33`, borderRadius: 8, fontSize: 12, color: TEXT, lineHeight: 1.55 }}>
          For your security, password changes and two-factor authentication are managed in your Clerk profile. Click a button above to open it in a new tab.
        </div>
      </Section>
    </div>
  )
