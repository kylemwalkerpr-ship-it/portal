...
const PAYOUT_STATUS = {
  ...
  skipped:     { bg:'#F2EFE9', text:'#9097A8', border:'rgba(0,0,0,.10)',   dot:'#9097A8', label:'Skipped'      },
}

const CONNECT_STATUS = {
  active:      { bg:'#EAF5EE', text:GREEN,  label:'Active'      },
  pending:     { bg:'#FEF5E4', text:AMBER,  label:'Pending'     },
  not_started: { bg:'#F2EFE9', text:'#9097A8', label:'Not started'},
}
...
const PayPill = ({status}) => { const c=PAYOUT_STATUS[status]||PAYOUT_STATUS.pending; return <span style={{display:'inline-flex',alignItems:'center',gap:'4px',padding:'3px 8px 3px 6px',borderRadius:'4px',fontSize:'11px',fontWeight:700,letterSpacing:'.04em',textTransform:'uppercase',background:c.bg,color:c.text,border:`1px solid ${c.border}`}}><span style={{width:'5px',height:'5px',borderRadius:'50%',background:c.dot,display:'inline-block'}}/>{c.label}</span>}
const ConnPill = ({status,bypass}) => { const c=CONNECT_STATUS[status]||CONNECT_STATUS.not_started; return <span style={{display:'inline-flex',alignItems:'center',gap:'4px',padding:'3px 8px',borderRadius:'4px',fontSize:'11px',fontWeight:700,background:c.bg,color:c.text}}>{bypass?'🔓 Bypass':c.label}</span>}

// ─── KPI card ─────────────────────────────────────────────────────────────────
const Kpi = ({label,value,sub,accent=NAVY,icon,onClick}) => {
  const [h,setH]=React.useState(false)
  return <div onMouseEnter={()=>setH(true)} onMouseLeave={()=>setH(false)} onClick={onClick} style={{background:'#fff',border:`1px solid ${h&&onClick?NAVY:'#DDD8CE'}`,borderTop:`3px solid ${accent}`,borderRadius:'8px',padding:'16px 18px',cursor:onClick?'pointer':'default',boxShadow:h&&onClick?'0 4px 12px rgba(27,45,79,.12)':'0 1px 3px rgba(27,45,79,.05)',transition:'all .15s',display:'flex',flexDirection:'column',gap:'5px'}}>
...
// ─── Section ──────────────────────────────────────────────────────────────────
const Sec = ({title,sub,right,children}) => <div style={{display:'flex',flexDirection:'column',gap:'12px'}}>
  <div style={{display:'flex',alignItems:'flex-end',justifyContent:'space-between',gap:'12px'}}>
    <div><h3 style={{fontFamily:serif,fontWeight:600,fontSize:'20px',color:NAVY,margin:0,letterSpacing:'-.01em'}}>{title}</h3>{sub&&<p style={{margin:'3px 0 0',fontSize:'13px',color:'#9097A8'}}>{sub}</p>}</div>
    {right}
  </div>
  {children}
</div>
...
// ─── Notice bar ───────────────────────────────────────────────────────────────
const Notice = ({msg,type='ok',onClose}) => msg ? <div style={{padding:'10px 16px',borderRadius:'7px',fontSize:'13px',fontWeight:600,background:type==='ok'?'#EAF5EE':'#FAEAEA',color:type==='ok'?GREEN:RED,border:`1px solid ${type==='ok'?'rgba(26,107,69,.20)':'rgba(139,26,26,.20)'}`,display:'flex',alignItems:'center',justifyContent:'space-between',gap:'8px'}}>
...
// ─── Confirm modal ────────────────────────────────────────────────────────────
const ConfirmModal = ({open,title,body,onConfirm,onClose,loading,danger=false}) => !open?null:(
  <div onClick={onClose} style={{position:'fixed',inset:0,background:'rgba(0,0,0,.55)',zIndex:400,display:'flex',alignItems:'center',justifyContent:'center',padding:'24px'}}>
    <div onClick={e=>e.stopPropagation()} style={{background:'#fff',borderRadius:'10px',padding:'28px',width:'100%',maxWidth:'420px',boxShadow:'0 20px 60px rgba(0,0,0,.22)',fontFamily:sans}}>
      ...
      <p style={{fontSize:'13px',color:'#5C6070',lineHeight:1.6,margin:'0 0 20px'}}>{body}</p>
      <div style={{display:'flex',gap:'8px',justifyContent:'flex-end'}}>
        <button onClick={onClose} disabled={loading} style={{padding:'8px 18px',borderRadius:'6px',border:'1px solid #DDD8CE',background:'#fff',color:'#5C6070',cursor:'pointer',fontSize:'13px',fontWeight:600,fontFamily:sans}}>Cancel</button>
...
// ─── MAIN ─────────────────────────────────────────────────────────────────────
export default function AdminPayouts({ formatPrimary }) {
                ...
                {[
                  ...
                  <div key={row.label} style={{display:'flex',alignItems:'center',justifyContent:'space-between',padding:'9px 0',borderBottom:'1px solid #F2EFE9'}}>
                    ...
                    <div style={{textAlign:'right'}}>
                      ...
                      <div style={{fontSize:'11px',color:'#9097A8'}}>{row.value} order{row.value!==1?'s':''}</div>
                    </div>
                  </div>
                ))}
              </Card>
          ...
          <div style={{display:'flex',gap:'10px',flexWrap:'wrap',alignItems:'center'}}>
            ...
            <div style={{display:'flex',gap:'6px'}}>
              {['all','pending','transferred','failed'].map(s=>(
                <button key={s} onClick={()=>setQueueFilter(s)} style={{padding:'7px 14px',borderRadius:'6px',border:`1px solid ${queueFilter===s?NAVY:'#DDD8CE'}`,background:queueFilter===s?NAVY:'#fff',color:queueFilter===s?'#fff':'#5C6070',cursor:'pointer',fontSize:'12px',fontWeight:600,fontFamily:sans,textTransform:'capitalize'}}>
                  {s==='all'?'All':s}
                </button>
              ))}
            </div>
                ...
                <tbody>
                  {queueData.loading ? [1,2,3,4,5].map(i=>(
                    <tr key={i} style={{background:i%2===0?'#fff':'#FAFAF8'}}>
                      {Array.from({length:10}).map((_,j)=><td key={j} style={{padding:'13px'}}><div style={{height:'14px',background:'#F2EFE9',borderRadius:'3px',width:j===0?'80%':'60%'}}/></td>)}
                    </tr>
                  )) : filteredQueue.length===0 ? (
                    <tr><td colSpan={10} style={{padding:'48px',textAlign:'center',color:'#9097A8',fontSize:'14px'}}>No orders match the current filter.</td></tr>
                  ) : filteredQueue.map((o,i)=>{
                    const isSelected=selectedRows.has(o.id)
                    return(
                      <tr key={o.id} style={{background:isSelected?'rgba(196,164,90,.07)':i%2===0?'#fff':'#FAFAF8',borderBottom:'1px solid #F2EFE9'}}>
                        ...
                        <td style={{padding:'11px 13px'}}><span style={{fontSize:'12px',color:'#9097A8',fontFamily:'monospace'}}>{o.id?.slice(0,8)}…</span></td>
                        <td style={{padding:'11px 13px'}}>
                          <div style={{fontWeight:600,fontSize:'13px',color:NAVY}}>{o.provider_name}</div>
                          <div style={{fontSize:'11px',color:'#9097A8'}}>{o.provider_role==='attorney'?'⚖️':'👤'} {o.provider_email}</div>
                        </td>
                        ...
                        <td style={{padding:'11px 13px',textAlign:'right',fontSize:'13px',color:'#9097A8',fontVariantNumeric:'tabular-nums'}}>{$(o.fee)}</td>
                        <td style={{padding:'11px 13px',fontSize:'12px',color:o.days_waiting>7?RED:o.days_waiting>3?AMBER:'#9097A8',fontWeight:o.days_waiting>7?700:400,whiteSpace:'nowrap'}}>
                          {o.days_waiting}d
                        </td>
                ...
                {filteredQueue.length>0&&!queueData.loading&&(
                  <tfoot>
                    <tr style={{background:'#F8F7F4',borderTop:'2px solid #DDD8CE'}}>
                      ...
                      <td style={{padding:'9px 13px',textAlign:'right',fontSize:'12px',fontWeight:700,color:'#9097A8'}}>
            ...
            <div style={{background:'#fff',border:'1px solid #DDD8CE',borderRadius:'8px',overflow:'hidden'}}>
              <div style={{overflowX:'auto'}}>
                <table style={{width:'100%',borderCollapse:'collapse',minWidth:'700px'}}>
                  ...
                  <tbody>
                    {connectData.loading ? [1,2,3].map(i=><tr key={i}><td colSpan={6} style={{padding:'14px'}}><div style={{height:'14px',background:'#F2EFE9',borderRadius:'3px'}}/></td></tr>)
                    : (connectData.data?.providers||[]).map((p,i)=>(
                      <tr key={p.provider_id} style={{background:i%2===0?'#fff':'#FAFAF8',borderBottom:'1px solid #F2EFE9'}}>
                        <td style={{padding:'11px 13px'}}>
                          <div style={{fontWeight:600,fontSize:'13px',color:NAVY}}>{p.name}</div>
                          <div style={{fontSize:'11px',color:'#9097A8'}}>{p.email}</div>
                        </td>
                        ...
                        <td style={{padding:'11px 13px'}}>
                          <label style={{display:'flex',alignItems:'center',gap:'6px',cursor:'pointer'}}>
                            ...
                            <span style={{fontSize:'12px',color:p.bypass?GOLD:'#9097A8',fontWeight:p.bypass?600:400}}>{p.bypass?'On':'Off'}</span>
                          </label>
                        </td>
                        <td style={{padding:'11px 13px'}}>
                          {p.payout_account&&(
