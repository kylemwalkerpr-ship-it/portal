...
export default function AdminEscrow() {
  ...
  const handleAction = (action,target) => {
    if(action==='release') return setModal({type:'release',order:target,title:'Force Release Escrow',body:`Release the full held amount (${$(target.escrow_amount)}) to ${target.provider_name||'the provider'}. This triggers an immediate Stripe transfer.`,inputs:[{name:'reason',label:'Reason',type:'textarea',placeholder:'e.g. Client confirmed delivery via email outside the platform'}]})
