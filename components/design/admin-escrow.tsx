...
const ESCROW_CFG = {
  ...
  refunded:         { dot:'var(--portal-ink-soft, #9097A8)', bg:'var(--portal-rule-soft, #F2EFE9)', text:'var(--portal-ink-soft, #9097A8)', label:'Refunded',  icon:'↩' },
}
...
const ago  = (s: any) => { if(!s) return '—'; const d=Math.floor((Date.now()-new Date(s).getTime())/86400000); return d===0?'Today':d===1?'Yesterday':d<30?`${d}d ago`:`${new Date(s).toLocaleDateString('en-GB',{day:'numeric',month:'short',year:'2-digit'})}` }
const inFuture = (s: any) => s && new Date(s) > new Date()
const daysUntil = (s: any) => { if(!s) return null; const d=Math.ceil((new Date(s).getTime()-Date.now())/86400000); return d }

const EscrowPill = ({status}: any) => { const c=ESCROW_CFG[status]||ESCROW_CFG.held; return <span style={{display:'inline-flex',alignItems:'center',gap:'4px',padding:'3px 8px 3px 6px',borderRadius:'4px',fontSize:'11px',fontWeight:700,letterSpacing:'.04em',textTransform:'uppercase',background:c.bg,color:c.text}}><span style={{width:'5px',height:'5px',borderRadius:'50%',background:c.dot,display:'inline-block',flexShrink:0}}/>{c.label}</span> }
...
// ─── Action modal ─────────────────────────────────────────────────────────────
const Modal = ({open,title,body,onConfirm,onClose,busy,danger=false,inputs=[]}: any) => {
  ...
  return(
    <div onClick={onClose} style={{position:'fixed',inset:0,background:'rgba(0,0,0,.55)',zIndex:400,display:'flex',alignItems:'center',justifyContent:'center',padding:'24px'}}>
      <div onClick={e=>e.stopPropagation()} style={{background:'#fff',borderRadius:'10px',padding:'24px',width:'100%',maxWidth:'460px',boxShadow:'0 20px 60px rgba(0,0,0,.22)',fontFamily:sans}}>
        ...
        <p style={{fontSize:'13px',color:'var(--portal-ink-mid, #5C6070)',lineHeight:1.55,margin:'0 0 16px'}}>{body}</p>
        {inputs.map(inp=>(
          <label key={inp.name} style={{display:'block',marginBottom:'12px'}}>
            <div style={{fontSize:'11px',fontWeight:700,color:'var(--portal-ink-mid, #5C6070)',textTransform:'uppercase',letterSpacing:'.05em',marginBottom:'5px'}}>{inp.label}{inp.required?' *':''}</div>
            {inp.type==='textarea'?(
        ...
        <div style={{display:'flex',gap:'8px',justifyContent:'flex-end'}}>
          <button onClick={onClose} disabled={busy} style={{padding:'8px 18px',borderRadius:'6px',border:'1px solid #DDD8CE',background:'#fff',color:'var(--portal-ink-mid, #5C6070)',cursor:'pointer',fontSize:'13px',fontWeight:600,fontFamily:sans}}>Cancel</button>
...
// ─── Hook: fetch ──────────────────────────────────────────────────────────────
function useFetch(url: string, deps: any[] = []) {
...
// ─── Detail drawer ────────────────────────────────────────────────────────────
function EscrowDrawer({order,onClose,onAction}: any) {
        ...
        <div style={{flex:1,overflowY:'auto',padding:'20px 24px'}}>
          ...
          {section==='overview'&&(
            <div style={{display:'flex',flexDirection:'column',gap:'14px'}}>
              {/* Money breakdown */}
              <div style={{background:'var(--portal-bg, #F7F5F0)',border:'1px solid #DDD8CE',borderRadius:'8px',padding:'16px'}}>
                ...
                {[
                  ...
                  {l:'Original Order Amount',v:$(order.gross),c:'var(--portal-ink-soft, #9097A8)'},
                ].map(r=>(
                  <div key={r.l} style={{display:'flex',justifyContent:'space-between',padding:'6px 0',borderBottom:'1px solid #E8E4DC'}}>
                    <span style={{fontSize:'13px',color:'var(--portal-ink-mid, #5C6070)'}}>{r.l}</span>
              ...
              <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'10px'}}>
                <div style={{background:'var(--portal-bg, #F7F5F0)',border:'1px solid #DDD8CE',borderRadius:'8px',padding:'12px 14px'}}>
                  <div style={{fontSize:'10px',fontWeight:700,color:'var(--portal-ink-soft, #9097A8)',textTransform:'uppercase',letterSpacing:'.06em',marginBottom:'6px'}}>Client</div>
                  <div style={{fontWeight:600,fontSize:'13px',color:NAVY}}>{order.client_name||'—'}</div>
                  <div style={{fontSize:'11px',color:'var(--portal-ink-soft, #9097A8)'}}>{order.client_email||'—'}</div>
                </div>
                <div style={{background:'var(--portal-bg, #F7F5F0)',border:'1px solid #DDD8CE',borderRadius:'8px',padding:'12px 14px'}}>
                  <div style={{fontSize:'10px',fontWeight:700,color:'var(--portal-ink-soft, #9097A8)',textTransform:'uppercase',letterSpacing:'.06em',marginBottom:'6px'}}>Provider</div>
                  <div style={{fontWeight:600,fontSize:'13px',color:order.provider_id?NAVY:'var(--portal-ink-soft, #9097A8)'}}>{order.provider_name||'Unassigned'}</div>
                  <div style={{fontSize:'11px',color:'var(--portal-ink-soft, #9097A8)'}}>{order.provider_email||'—'}</div>
                </div>
              </div>
              ...
              {escrowStatus==='disputed'&&order.escrow_dispute_reason&&(
                <div style={{background:'#FAEAEA',border:'1px solid rgba(139,26,26,.22)',borderRadius:'8px',padding:'14px 16px'}}>
                  ...
                  <div style={{fontSize:'13px',color:'var(--portal-ink-mid, #5C6070)',lineHeight:1.55}}>{order.escrow_dispute_reason}</div>
                </div>
              )}
              {escrowStatus==='frozen'&&order.escrow_frozen_reason&&(
                <div style={{background:'#F5F3FF',border:'1px solid rgba(61,43,107,.22)',borderRadius:'8px',padding:'14px 16px'}}>
                  ...
                  <div style={{fontSize:'13px',color:'var(--portal-ink-mid, #5C6070)',lineHeight:1.55}}>{order.escrow_frozen_reason}</div>
                </div>
              )}
              {/* Action buttons */}
              <div style={{background:'var(--portal-bg, #F7F5F0)',border:'1px solid #DDD8CE',borderRadius:'8px',padding:'14px 16px'}}>
                <div style={{fontSize:'10px',fontWeight:700,color:'var(--portal-ink-soft, #9097A8)',textTransform:'uppercase',letterSpacing:'.06em',marginBottom:'10px'}}>Admin Actions</div>
          ...
          {section==='milestones'&&(
            <div style={{display:'flex',flexDirection:'column',gap:'10px'}}>
              ...
              {milestones===null?<p style={{color:'var(--portal-ink-soft, #9097A8)',fontSize:'13px',padding:'16px',textAlign:'center'}}>Loading milestones…</p>:milestones.length===0?<div style={{background:'var(--portal-bg, #F7F5F0)',border:'1px dashed #C8C2B6',borderRadius:'8px',padding:'24px',textAlign:'center'}}><div style={{fontSize:'28px',marginBottom:'8px',opacity:.4}}>🎯</div><div style={{fontFamily:serif,fontWeight:600,fontSize:'16px',color:NAVY}}>No milestones yet</div><p style={{fontSize:'12px',color:'var(--portal-ink-soft, #9097A8)',margin:'4px 0 0',lineHeight:1.5}}>Milestones split escrow into staged releases. Add one for projects with deliverable phases.</p></div>:milestones.map(m=>(
                <div key={m.id} style={{background:'var(--portal-bg, #F7F5F0)',border:'1px solid #DDD8CE',borderRadius:'8px',padding:'14px 16px'}}>
                  <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',marginBottom:'6px'}}>
                    <div><span style={{fontSize:'10px',fontWeight:700,color:'var(--portal-ink-soft, #9097A8)',textTransform:'uppercase',letterSpacing:'.06em'}}>Milestone {m.sequence}</span><div style={{fontWeight:700,fontSize:'14px',color:NAVY}}>{m.title}</div></div>
                    <div style={{textAlign:'right'}}><div style={{fontWeight:800,fontSize:'16px',color:NAVY}}>{$(m.amount)}</div><span style={{fontSize:'10px',fontWeight:700,letterSpacing:'.05em',textTransform:'uppercase',color:m.status==='released'?GREEN:m.status==='approved'?CYAN:m.status==='rejected'?RED:'var(--portal-ink-soft, #9097A8)'}}>{m.status?.replace('_',' ')}</span></div>
                  </div>
                  {m.description&&<div style={{fontSize:'12px',color:'var(--portal-ink-mid, #5C6070)',lineHeight:1.55,marginBottom:'8px'}}>{m.description}</div>}
                  {m.due_date&&<div style={{fontSize:'11px',color:'var(--portal-ink-soft, #9097A8)'}}>Due {new Date(m.due_date).toLocaleDateString('en-GB',{day:'numeric',month:'short',year:'2-digit'})}</div>}
          ...
          {section==='scope'&&(
            <div style={{display:'flex',flexDirection:'column',gap:'10px'}}>
              {scopeChanges===null?<p style={{color:'var(--portal-ink-soft, #9097A8)',fontSize:'13px',padding:'16px',textAlign:'center'}}>Loading scope changes…</p>:scopeChanges.length===0?<div style={{background:'var(--portal-bg, #F7F5F0)',border:'1px dashed #C8C2B6',borderRadius:'8px',padding:'24px',textAlign:'center'}}><div style={{fontSize:'28px',marginBottom:'8px',opacity:.4}}>📐</div><div style={{fontFamily:serif,fontWeight:600,fontSize:'16px',color:NAVY}}>No scope changes</div><p style={{fontSize:'12px',color:'var(--portal-ink-soft, #9097A8)',margin:'4px 0 0',lineHeight:1.5}}>Providers can request escrow increases when revisions exceed original scope. Client must approve before funds move.</p></div>:scopeChanges.map(sc=>(
                <div key={sc.id} style={{background:'var(--portal-bg, #F7F5F0)',border:'1px solid #DDD8CE',borderRadius:'8px',padding:'14px 16px'}}>
                  <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',marginBottom:'6px'}}>
                    <div style={{flex:1,minWidth:0}}>
                      <span style={{fontSize:'10px',fontWeight:700,color:'var(--portal-ink-soft, #9097A8)',textTransform:'uppercase',letterSpacing:'.06em'}}>{sc.change_type?.replace('_',' ')} · {sc.requested_by_role}</span>
                      <div style={{fontWeight:700,fontSize:'14px',color:NAVY}}>{sc.amount_delta>=0?'+':''}{$(sc.amount_delta)}</div>
                    </div>
                    <span style={{fontSize:'10px',fontWeight:700,letterSpacing:'.05em',textTransform:'uppercase',color:sc.status==='approved'?GREEN:sc.status==='rejected'?RED:sc.status==='expired'?'var(--portal-ink-soft, #9097A8)':AMBER}}>{sc.status}</span>
                  </div>
                  <div style={{fontSize:'12px',color:'var(--portal-ink-mid, #5C6070)',lineHeight:1.55,marginBottom:'8px'}}>{sc.reason}</div>
                  <div style={{fontSize:'11px',color:'var(--portal-ink-soft, #9097A8)'}}>Requested {ago(sc.created_at)}{sc.client_decision_at&&` · Client decided ${ago(sc.client_decision_at)}`}</div>
                  {sc.status==='pending'&&(
                    <div style={{display:'flex',gap:'6px',marginTop:'10px'}}>
          ...
          {section==='events'&&(
            <div style={{display:'flex',flexDirection:'column',gap:'8px'}}>
              <div style={{fontSize:'12px',color:'var(--portal-ink-soft, #9097A8)',marginBottom:'4px'}}>Every escrow state change, in chronological order.</div>
              {events===null?<p style={{color:'var(--portal-ink-soft, #9097A8)',fontSize:'13px',padding:'16px',textAlign:'center'}}>Loading…</p>:events.length===0?<p style={{color:'var(--portal-ink-soft, #9097A8)',fontSize:'13px',padding:'16px',textAlign:'center',background:'var(--portal-bg, #F7F5F0)',borderRadius:'8px'}}>No escrow events yet.</p>:events.map((e,i)=>(
                <div key={i} style={{background:'var(--portal-bg, #F7F5F0)',border:'1px solid #DDD8CE',borderRadius:'7px',padding:'10px 14px'}}>
                  <div style={{display:'flex',justifyContent:'space-between',gap:'8px',marginBottom:'3px'}}>
                    <span style={{fontWeight:700,fontSize:'13px',color:NAVY,textTransform:'capitalize'}}>{e.event_type?.replace(/_/g,' ')}</span>
                    <span style={{fontSize:'11px',color:'var(--portal-ink-soft, #9097A8)'}}>{ago(e.created_at)}</span>
                  </div>
                  ...
                  {e.reason&&<div style={{fontSize:'12px',color:'var(--portal-ink-mid, #5C6070)',lineHeight:1.5,marginTop:'2px'}}>{e.reason}</div>}
                  <div style={{fontSize:'10px',color:'var(--portal-ink-soft, #9097A8)',marginTop:'3px'}}>{e.actor_role||'system'}</div>
                </div>
              ))}
            </div>
          )}
...
export default function AdminEscrow() {
  ...
  const runAction = async(action,target,extraValues={}) => {
    ...
    try{
      ...
      else if(action==='add_milestone')await post(`/api/admin/escrow/${target.id}/milestones`,{...extraValues,amount:Number((extraValues as any).amount),sequence:1})
  ...
  const thStyle=(col: any): React.CSSProperties=>({padding:'10px 12px',textAlign:'left',fontSize:'11px',fontWeight:700,color:sortCol===col?'#C4A45A':'rgba(255,255,255,.68)',background:NAVY,whiteSpace:'nowrap',cursor:'pointer',letterSpacing:'.06em',textTransform:'uppercase',borderBottom:'2px solid rgba(255,255,255,.08)',userSelect:'none'})
  ...
  return(
    <div style={{padding:'28px',display:'flex',flexDirection:'column',gap:'22px',fontFamily:sans,background:'var(--portal-bg, #F7F5F0)',minHeight:'100vh'}}>
      ...
      <div style={{display:'flex',alignItems:'flex-end',justifyContent:'space-between',gap:'12px',flexWrap:'wrap'}}>
        <div>
          <div style={{fontSize:'11px',fontWeight:700,textTransform:'uppercase',letterSpacing:'.14em',color:'var(--portal-ink-soft, #9097A8)',marginBottom:'4px'}}>Money in Motion</div>
          ...
          <p style={{color:'var(--portal-ink-soft, #9097A8)',fontSize:'13px',margin:'5px 0 0'}}>Automated state machine. Mirror visibility to clients. Override every scenario.</p>
        </div>
      ...
      {tab==='overview'&&(
        <>
          <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(180px,1fr))',gap:'12px'}}>
            ...
            <Kpi label="Disputed" value={summary.disputed_count||0} sub={$(summary.disputed_total)} accent={summary.disputed_count>0?RED:'var(--portal-ink-soft, #9097A8)'} icon="⚠" onClick={()=>setTab('disputed')}/>
            <Kpi label="Frozen" value={summary.frozen_count||0} sub={$(summary.frozen_total)} accent={summary.frozen_count>0?PURPLE:'var(--portal-ink-soft, #9097A8)'} icon="🧊" onClick={()=>setTab('frozen')}/>
            ...
            <Kpi label="Refunded" value={$(summary.refunded_total)} sub={`${summary.refunded_count||0} orders`} accent={'var(--portal-ink-soft, #9097A8)'} icon="↩" onClick={()=>setTab('refunded')}/>
          </div>
          ...
          {ledgerData&&(
            <div>
              <div style={{display:'flex',alignItems:'center',gap:'6px',marginBottom:'10px'}}>
                <span style={{fontSize:'11px',fontWeight:700,textTransform:'uppercase',letterSpacing:'.12em',color:'var(--portal-ink-soft, #9097A8)'}}>Canonical Ledger Check</span>
                <span style={{fontSize:'10px',padding:'2px 6px',borderRadius:'4px',background:'#EAF5EE',color:'var(--portal-moss, #1A6B45)',fontWeight:700}}>live</span>
              </div>
              ...
              <div style={{fontSize:'11px',color:'var(--portal-ink-soft, #9097A8)',marginTop:'6px',lineHeight:1.5,paddingLeft:'2px'}}>
                Totals from <code style={{fontSize:'10px',background:'var(--portal-rule-soft, #F2EFE9)',padding:'1px 5px',borderRadius:'3px'}}>canonical_ledger</code> — {ledgerData.escrow_held_count||0} deposits, {ledgerData.escrow_released_count||0} releases, {ledgerData.escrow_refunded_count||0} refunds
              </div>
            </div>
          )}
          ...
          <Card style={{padding:'18px 20px',borderLeft:`4px solid ${GOLD}`}}>
            ...
            <ol style={{margin:0,paddingLeft:'18px',fontSize:'13px',color:'var(--portal-ink-mid, #5C6070)',lineHeight:1.7}}>
      ...
      {tab!=='overview'&&(
        <>
          ...
          {tab==='auto'&&(
            <div>
              <div style={{display:'flex',alignItems:'center',gap:'8px',marginBottom:'10px'}}>
                <span style={{fontSize:'11px',fontWeight:700,textTransform:'uppercase',letterSpacing:'.12em',color:'var(--portal-ink-soft, #9097A8)'}}>Cron Job Monitor</span>
                <span style={{fontSize:'10px',padding:'2px 6px',borderRadius:'4px',background:loadingAudit?'var(--portal-rule-soft, #F2EFE9)':'#EAF5EE',color:loadingAudit?'var(--portal-ink-soft, #9097A8)':'var(--portal-moss, #1A6B45)',fontWeight:700,transition:'all .2s'}}>{loadingAudit?'loading…':'admin_audit_log'}</span>
              </div>
              {loadingAudit&&!auditEntries?(
                <div style={{background:'#fff',border:'1px solid #DDD8CE',borderRadius:'8px',padding:'24px',textAlign:'center',fontSize:'13px',color:'var(--portal-ink-soft, #9097A8)'}}>Loading sweep history…</div>
                ...
                <div style={{background:'#fff',border:'1px dashed #C8C2B6',borderRadius:'8px',padding:'20px',textAlign:'center'}}>
                  ...
                  <div style={{fontSize:'12px',color:'var(--portal-ink-soft, #9097A8)',lineHeight:1.5}}>
                    No entries in <code style={{fontSize:'11px',background:'var(--portal-rule-soft, #F2EFE9)',padding:'1px 5px',borderRadius:'3px'}}>admin_audit_log</code> for auto-release sweeps.
                      ...
                      <div key={e.id||i} style={{display:'flex',alignItems:'center',gap:'12px',background:'#fff',border:'1px solid #DDD8CE',borderRadius:'7px',padding:'11px 14px',transition:'all .12s'}}>
                        ...
                        <div style={{flex:'1',minWidth:0}}>
                          <div style={{display:'flex',alignItems:'center',gap:'6px',flexWrap:'wrap'}}>
                            ...
                            {releasedCount!==null&&<span style={{padding:'1px 6px',borderRadius:'4px',background:releasedCount>0?'#EAF5EE':'var(--portal-rule-soft, #F2EFE9)',color:releasedCount>0?GREEN:'var(--portal-ink-soft, #9097A8)',fontSize:'10px',fontWeight:700}}>{fmtN(releasedCount)} released</span>}
                            {!isCron&&<span style={{fontSize:'10px',padding:'1px 5px',borderRadius:'3px',background:'rgba(15,23,42,.06)',color:'var(--portal-ink-mid, #5C6070)',fontWeight:500}}>by admin</span>}
                          </div>
                          <div style={{display:'flex',gap:'10px',flexWrap:'wrap',marginTop:'3px'}}>
                            {releasedTotal!==null&&<span style={{fontSize:'11px',color:'var(--portal-ink-mid, #5C6070)'}}>Total: <strong style={{color:GREEN}}>{$(releasedTotal)}</strong></span>}
                            {earningsReleased!==null&&<span style={{fontSize:'11px',color:'var(--portal-ink-mid, #5C6070)'}}>Earnings released: <strong style={{color:NAVY}}>{fmtN(earningsReleased)}</strong></span>}
                            <span style={{fontSize:'11px',color:'var(--portal-ink-soft, #9097A8)'}}>{ago(e.ran_at)}</span>
                          </div>
                        </div>
                        <div style={{flexShrink:0,fontSize:'10px',color:'var(--portal-ink-soft, #9097A8)',fontWeight:600,fontFamily:'monospace'}}>{new Date(e.ran_at).toLocaleString('en-US',{month:'short',day:'numeric',hour:'2-digit',minute:'2-digit'})}</div>
                      </div>
                    )
                  })}
                  {auditEntries.length>10&&(
                    <div style={{textAlign:'center',fontSize:'12px',color:'var(--portal-ink-soft, #9097A8)',padding:'8px 0'}}>
          ...
          <div style={{display:'flex',gap:'10px',flexWrap:'wrap',alignItems:'center'}}>
            ...
            <span style={{marginLeft:'auto',fontSize:'12px',color:'var(--portal-ink-soft, #9097A8)'}}>{serverTotal.toLocaleString()} orders</span>
          </div>
            ...
            <div style={{background:'#fff',border:'1px solid #DDD8CE',borderRadius:'8px',overflow:'hidden',boxShadow:'0 1px 4px rgba(27,45,79,.06)'}}>
              <div style={{overflowX:'auto'}}>
                <table style={{width:'100%',borderCollapse:'collapse',minWidth:'1000px'}}>
                  ...
                  <tbody>
                    {list.loading?[1,2,3,4,5].map(i=><tr key={i} style={{background:i%2===0?'#fff':'var(--portal-surface-2, #FAFAF8)'}}>{Array.from({length:10}).map((_,j)=><td key={j} style={{padding:'13px'}}><div style={{height:'13px',background:'var(--portal-rule-soft, #F2EFE9)',borderRadius:'3px',width:j===0?'80%':'55%'}}/></td>)}</tr>)
                    :orders.length===0?<tr><td colSpan={10} style={{padding:'48px',textAlign:'center',color:'var(--portal-ink-soft, #9097A8)',fontSize:'14px'}}>No orders match this filter.</td></tr>
                    :orders.map((o,i)=>{
                      const eligibleNow = o.auto_release_eligible_at && !inFuture(o.auto_release_eligible_at)
                      const daysAge = o.days_in_escrow ?? Math.floor((Date.now()-new Date(o.status_updated_at||o.created_at).getTime())/86400000)
                      return(
                        <tr key={o.id} style={{background:i%2===0?'#fff':'var(--portal-surface-2, #FAFAF8)',borderBottom:'1px solid #F2EFE9'}}>
                          ...
                          <td style={{padding:'11px 12px',fontSize:'13px',color:'var(--portal-ink-mid, #5C6070)',whiteSpace:'nowrap'}}>{o.client_name||'—'}</td>
                          <td style={{padding:'11px 12px',fontSize:'13px',color:o.provider_id?NAVY:'var(--portal-ink-soft, #9097A8)',fontStyle:o.provider_id?'normal':'italic'}}>{o.provider_name||'Unassigned'}</td>
                          <td style={{padding:'11px 12px'}}><EscrowPill status={o.escrow_status||'held'}/></td>
                          <td style={{padding:'11px 12px',textAlign:'right',fontWeight:700,fontSize:'13px',color:Number(o.escrow_amount)>0?AMBER:'var(--portal-ink-soft, #9097A8)',fontVariantNumeric:'tabular-nums'}}>{$(o.escrow_amount)}</td>
                          ...
                          <td style={{padding:'11px 12px',fontSize:'12px',whiteSpace:'nowrap'}}>
                            {o.auto_release_eligible_at?(
                              eligibleNow?<span style={{color:GOLD,fontWeight:700}}>NOW</span>:<span style={{color:'var(--portal-ink-soft, #9097A8)'}}>{daysUntil(o.auto_release_eligible_at)}d</span>
                            ):<span style={{color:'var(--portal-ink-soft, #9097A8)'}}>—</span>}
                          </td>
                          <td style={{padding:'11px 12px',fontSize:'12px',color:daysAge>14?RED:daysAge>7?AMBER:'var(--portal-ink-soft, #9097A8)',whiteSpace:'nowrap'}}>{daysAge}d</td>
                          <td style={{padding:'11px 12px',whiteSpace:'nowrap'}}>
                            <div style={{display:'flex',gap:'5px'}}>
                              <button onClick={()=>setSelectedOrder(o)} style={{padding:'5px 10px',borderRadius:'5px',border:'1px solid #DDD8CE',background:'var(--portal-bg, #F7F5F0)',color:NAVY,cursor:'pointer',fontSize:'12px',fontWeight:600,fontFamily:sans}}>Manage</button>
                            </div>
                          </td>
                        </tr>
                      )
                  ...
                  {orders.length>0&&!list.loading&&(
                    <tfoot><tr style={{background:'#F8F7F4',borderTop:'2px solid #DDD8CE'}}>
                      <td colSpan={5} style={{padding:'9px 12px',fontSize:'12px',color:'var(--portal-ink-soft, #9097A8)',fontWeight:600}}>{(page-1)*PER_PAGE+1}–{Math.min(page*PER_PAGE,serverTotal)} of {serverTotal.toLocaleString()}</td>
              ...
              {/* Pagination */}
              {totalPages>1&&(
                <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',padding:'12px 16px',borderTop:'1px solid #F2EFE9',background:'var(--portal-surface-2, #FAFAF8)'}}>
                  <button onClick={()=>setPage(p=>Math.max(1,p-1))} disabled={page===1} style={{padding:'6px 14px',borderRadius:'6px',border:'1px solid #DDD8CE',background:page===1?'var(--portal-bg, #F7F5F0)':'#fff',color:page===1?'var(--portal-ink-soft, #9097A8)':NAVY,cursor:page===1?'not-allowed':'pointer',fontSize:'13px',fontWeight:600,fontFamily:sans}}>← Prev</button>
                  <span style={{fontSize:'13px',color:'var(--portal-ink-mid, #5C6070)'}}>Page {page} of {totalPages}</span>
                  <button onClick={()=>setPage(p=>Math.min(totalPages,p+1))} disabled={page===totalPages} style={{padding:'6px 14px',borderRadius:'6px',border:'1px solid #DDD8CE',background:page===totalPages?'var(--portal-bg, #F7F5F0)':'#fff',color:page===totalPages?'var(--portal-ink-soft, #9097A8)':NAVY,cursor:page===totalPages?'not-allowed':'pointer',fontSize:'13px',fontWeight:600,fontFamily:sans}}>Next →</button>
                </div>
              )}
            </div>
          )}
