...
describe('Bing SEO warning regressions', () => {
  test('public File Shop and Marketplace images carry meaningful alt text', () => {
    ...
    expect(shop).toContain('alt={`${product.title} file cover`}')
    expect(shop).not.toContain('src={product.cover} alt=""')
    expect(rail).toContain('alt={`${p.title} file cover`}')
