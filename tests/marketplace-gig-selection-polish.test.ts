...
describe('Marketplace selected gig and browse-grid polish', () => {
  ...
  test('keeps marketplace browse grids four-up on desktop without stretching incomplete rows', () => {
    const gridStart = css.indexOf('.cw-market .gig-grid {')
    expect(gridStart).toBeGreaterThanOrEqual(0)
    const gridCss = css.slice(gridStart)
    expect(gridCss).toContain('display: grid !important')
    expect(gridCss).toContain('grid-template-columns: repeat(4, minmax(0, 1fr)) !important')
    expect(gridCss).toContain('grid-template-columns: repeat(3, minmax(0, 1fr)) !important')
    expect(gridCss).toContain('grid-template-columns: repeat(2, minmax(0, 1fr)) !important')
    expect(gridCss).toContain('grid-template-columns: minmax(0, 1fr) !important')
    expect(gridCss).not.toContain('flex: 1 1 calc(')
    expect(gridCss).not.toContain('flex-wrap: wrap !important')
  })
})
