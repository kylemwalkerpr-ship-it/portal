...
describe('recovered Cloudflare shop additions', () => {
  ...
  test('keeps the established 20-product + 16-pack card contract while layering audited commercial state', () => {
    ...
    expect(shopUi).toContain('Search visa packs, planners, business tools…')
    expect(shopUi).toContain('Shop by category')
    expect(shopUi).toContain('Browse all {publishedProducts.length} files')
