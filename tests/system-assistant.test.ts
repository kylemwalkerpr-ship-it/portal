...
describe('system-wide assistant model routing', () => {
  ...
  it('routes through Messenger SuperGrok auth/model without another AI provider', async () => {
    ...
    expect(String((fetchMock.mock.calls as any[][])[0][0])).toBe('https://system-ai.example/v1/chat/completions')
  })
  ...
  it('uses an independent protocol fallback after a primary timeout', async () => {
    ...
    expect(String((fetchMock.mock.calls as any[][])[0][0])).toContain('/chat/completions')
    expect(String((fetchMock.mock.calls as any[][])[1][0])).toContain('/responses')
  })
})
