...
describe('persisted GSC visibility measurement', () => {
  ...
  it('keeps contact-detail / year-suffixed self-brand rows observable as junk (final review residual)', () => {
    // Final review residual: brand self-searches with a contact detail or a
    // calendar year were still measured as qualified demand. They stay raw
    // observations — counted in the junk bucket — while brand-like prose keeps
    // its genuine classification.
    const rows: FakeRow[] = [
      {
    ...
    const annotated = annotatePersistedGscRows(rows)
    expect(annotated.map((row) => row.visibilityClass)).toEqual(['junk', 'junk', 'qualified'])
    // Raw rows stay raw: the brand observations are preserved for measurement.
    expect(annotated[0].query).toBe('you safe contact number')
    expect(annotated[1].query).toBe('you safe login 2024')

    const summary = buildGscVisibilitySummary({ rows, displayLimit: 100, windowDays: 90 })
    expect(summary.rowCount).toBe(3)
    expect(summary.totals.impressions).toBe(860)
    expect(summary.junk.rowCount).toBe(2)
    expect(summary.junk.impressions).toBe(680)
    expect(summary.qualified.rowCount).toBe(1)
    expect(summary.qualified.impressions).toBe(180)
    expect(summary.offMission.rowCount).toBe(0)
  })
