...
describe('Entrim Qwen3.8 27B catalogue wiring', () => {
  ...
  it('catalog parseStudioPin resolves the studio picker selection', () => {
    ...
    expect(parsed.model.id).toBe('qwen3.8-27b')
    expect(parsed.host.id).toBe('entrim')
    expect(parsed.model.apiModel).toBe('Qwen/Qwen3.8-27B')
  })
  ...
  it('aliases resolve to the canonical pin', () => {
    ...
    expect(canonicalizePin('qwen3.8-27b')).toBe('entrim-qwen-27b')
  ...
  it('consts point at the exact upstream model id', () => {
    ...
    expect(ENTRIM_QWEN_MODEL).toBe('Qwen/Qwen3.8-27B')
